...
=== FILE MAP (regen: tools/gen_section_index.py) ===
  ...
  L1183   Config drift guard + queue-drain + host normalization
  L1836   Ollama num_ctx, host failover, per-host models, machines
  L2244   Remote executor + SSH transport + RemoteWorkspace + Windows
  L3508   Session lifecycle: autosave, snapshots, statusline
  L3777   Composer: input buffer, queue, paste/key parsing
  L4334   Version/update precedence + provider registration API
  L4977   Session modes + CLI arg completion + /set + leash + per-model
  L5447   Grouped lean-tools + provider/tool discovery dirs
  L5740   Hygiene sweep + FILE MAP drift gate (final gates)
=== END FILE MAP ===
"""
...
_nondefault = {
    ...
    "auto_compact": False,
    "compact_soft": 0.42, "compact_at": 0.61, "compact_gap": 0.15, "compact_emergency": 0.99,
...
# 10c-bis. compact_at/compact_gap drive compact_soft (one lever moves the pair;
...
check("compact: /set compact_at slides soft (soft = at - gap)",
...
check("compact: /set compact_gap re-derives soft from compact_at",
...
check("compact thresholds default 0.7/0.90 + emergency 1.0 (compact_at down from 0.95)",
      lc.Config().compact_soft == 0.70 and lc.Config().compact_at == 0.90
