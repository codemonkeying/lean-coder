...
=== FILE MAP (regen: tools/gen_section_index.py) ===
  ...
  L2365   Config drift guard + queue-drain + host normalization
  L3384   Ollama num_ctx, host failover, per-host models, machines
  L3806   Remote executor + SSH transport + RemoteWorkspace + Windows
  L5197   Session lifecycle: autosave, snapshots, statusline
  L5529   Composer: input buffer, queue, paste/key parsing
  L6138   Version/update precedence + provider registration API
  L6813   Session modes + CLI arg completion + /set + leash + per-model
  L7300   Grouped lean-tools + provider/tool discovery dirs
  L7604   Per-process run dirs (liveness reaper, no age)
  L7634   Hygiene sweep + FILE MAP drift gate (final gates)
=== END FILE MAP ===
"""
...
check("list_files cap names the follow-up (list_files path=<subdir>)",
      "list_files path=<a dir above>" in _capt.list_files())
