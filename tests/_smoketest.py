...
=== FILE MAP (regen: tools/gen_section_index.py) ===
  ...
  L1331   Config drift guard + queue-drain + host normalization
  L2014   Ollama num_ctx, host failover, per-host models, machines
  L2422   Remote executor + SSH transport + RemoteWorkspace + Windows
  L3720   Session lifecycle: autosave, snapshots, statusline
  L3989   Composer: input buffer, queue, paste/key parsing
  L4546   Version/update precedence + provider registration API
  L5207   Session modes + CLI arg completion + /set + leash + per-model
  L5691   Grouped lean-tools + provider/tool discovery dirs
  L5984   Hygiene sweep + FILE MAP drift gate (final gates)
=== END FILE MAP ===
"""
...
_dwcfg.worker_max_depth = 1
_dwcfg.worker_max_children = 0
# shared TOKEN budget (the boss's slow-bleed guard): meter finished-worker .usage spend.
_tbtmp = Path(tempfile.mkdtemp(prefix="lc_tokbudget_"))
_tbres = _tbtmp / "wt.result"; _tbbrief = _tbtmp / "wt.brief"
(_tbtmp / "wt.brief.usage").write_text("in=1200 out=800\n")
check("token-budget: _read_usage parses the .usage sidecar",
      _dw._read_usage(str(_tbres)) == (1200, 800))
check("token-budget: _read_usage of a missing sidecar -> (0,0)",
...
check("token-budget: _accrue_usage adds to session totals",
...
check("token-budget: _accrue_usage is idempotent (no double count)",
      _dw._worker_tokens_spent() == 2000)
# the dispatch gate refuses once realized spend crosses the ceiling
_dwcfg.worker_token_budget = 1500
import os as _os_tb
_os_tb.environ.pop("LEANCODER_WORKER_TOKEN_BUDGET", None)
check("token-budget: dispatch refused once budget spent",
      "token budget is spent" in _dw.run({"task": "another"}, str(FIX)))
_dwcfg.worker_token_budget = 0   # 0 = unlimited: gate is inert
_dw._H["tokens_in"] = 0; _dw._H["tokens_out"] = 0
_dw._H["workers"] = {}
