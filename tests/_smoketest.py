...
=== FILE MAP (regen: tools/gen_section_index.py) ===
  ...
  L2692   Ollama num_ctx, host failover, per-host models, machines
  L3100   Remote executor + SSH transport + RemoteWorkspace + Windows
  L4439   Session lifecycle: autosave, snapshots, statusline
  L4708   Composer: input buffer, queue, paste/key parsing
  L5265   Version/update precedence + provider registration API
  L5926   Session modes + CLI arg completion + /set + leash + per-model
  L6413   Grouped lean-tools + provider/tool discovery dirs
  L6719   Hygiene sweep + FILE MAP drift gate (final gates)
=== END FILE MAP ===
"""
...
# Termux: composer defaults OFF (its terminal snaps to cursor -> the pinned-input
# scroll-region repaint yanks scrollback to the bottom every streamed token). Only a
# DEFAULT: an explicit `composer` in config still wins. Isolate on a scratch config.
_tx_orig_cp = lc.CONFIG_PATH
_tx_env = {k: os.environ.get(k) for k in ("TERMUX_VERSION", "PREFIX")}
try:
    os.environ["TERMUX_VERSION"] = "0.118.0"
    os.environ.pop("PREFIX", None)
    check("_is_termux: TERMUX_VERSION set -> True", lc._is_termux() is True)
    _tx_cp = Path(tempfile.mkdtemp()) / "notx.toml"
    _tx_cp.write_text('model = "x"\n')          # no composer key -> smart default applies
    lc.CONFIG_PATH = _tx_cp
    check("termux: composer defaults OFF when config doesn't set it",
          lc.load_config(_Args()).composer is False)
    _tx_cp.write_text('model = "x"\ncomposer = true\n')  # explicit -> wins over the default
    check("termux: an explicit composer=true still wins on Termux",
          lc.load_config(_Args()).composer is True)
    os.environ.pop("TERMUX_VERSION", None)
    check("_is_termux: no Termux env -> False", lc._is_termux() is False)
    _tx_cp.write_text('model = "x"\n')
    check("non-termux: composer keeps its default (on) when config is silent",
          lc.load_config(_Args()).composer is True)
finally:
    lc.CONFIG_PATH = _tx_orig_cp
    for _k, _v in _tx_env.items():
        if _v is None:
