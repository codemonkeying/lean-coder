...
=== FILE MAP (regen: tools/gen_section_index.py) ===
  ...
  L1675   Config drift guard + queue-drain + host normalization
  L2386   Ollama num_ctx, host failover, per-host models, machines
  L2794   Remote executor + SSH transport + RemoteWorkspace + Windows
  L4092   Session lifecycle: autosave, snapshots, statusline
  L4361   Composer: input buffer, queue, paste/key parsing
  L4918   Version/update precedence + provider registration API
  L5579   Session modes + CLI arg completion + /set + leash + per-model
  L6063   Grouped lean-tools + provider/tool discovery dirs
  L6356   Hygiene sweep + FILE MAP drift gate (final gates)
=== END FILE MAP ===
"""
...
check("board tool: find matches by text",
      "t2" in _plain(_bt.run({"action": "find", "board": "swarm", "query": "ui"}, "/tmp")))
check("board tool: find with no match says so",
      "no task matches" in _bt.run({"action": "find", "board": "swarm", "query": "zzz"}, "/tmp"))
...
check("board tool: a worker cannot assign",
      "driver-only" in _bt.run({"action": "assign", "board": "swarm", "task": "t2",
                                "worker": "9"}, "/tmp"))
check("board tool: a worker cannot block",
      "driver-only" in _bt.run({"action": "block", "board": "swarm", "task": "t2"}, "/tmp"))
