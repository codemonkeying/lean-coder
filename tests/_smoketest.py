...
=== FILE MAP (regen: tools/gen_section_index.py) ===
  L51     Tool schemas, note tool, command/help contract
  L420    Core tool bodies (ignore, diff parser, read/write/apply, search, run_command)
  L2157   Config drift guard + queue-drain + host normalization
  L3050   Ollama num_ctx, host failover, per-host models, machines
  L3472   Remote executor + SSH transport + RemoteWorkspace + Windows
  L4827   Session lifecycle: autosave, snapshots, statusline
  L5146   Composer: input buffer, queue, paste/key parsing
  L5755   Version/update precedence + provider registration API
  L6425   Session modes + CLI arg completion + /set + leash + per-model
  L6912   Grouped lean-tools + provider/tool discovery dirs
  L7218   Hygiene sweep + FILE MAP drift gate (final gates)
=== END FILE MAP ===
"""
...
# ...and the FULL shipped surface (core + EVERY bundled lean-tool loaded). The README
# claims ~6k for this; gate it so that number can't silently drift. NOT the always-on
# baseline (lean-tools are opt-in) - this is the ceiling if you /tools everything on.
...
check("full surface (core + all bundled lean-tools) < 6500 tokens (~6k README claim + headroom)",
      full_overhead < 6500, f"~{full_overhead} tokens")
...
open(_susp_stamp + ".brief.checkpoint", "w").write("ck")
open(_susp_stamp + ".brief.suspended", "w").write("1")
lc._clean_worker_sidecars(_susp_stamp + ".brief")
check("reaper: suspended worker's checkpoint survives cleanup",
      _os_dw.path.exists(_susp_stamp + ".brief.checkpoint") and lc._worker_suspended(_susp_stamp + ".brief"))
# without the sentinel the same call wipes the family
_os_dw.remove(_susp_stamp + ".brief.suspended")
lc._clean_worker_sidecars(_susp_stamp + ".brief")
check("reaper: without sentinel the checkpoint is cleaned",
      not _os_dw.path.exists(_susp_stamp + ".brief.checkpoint"))
...
open(_p1 + ".brief.checkpoint", "w").write("ck1")
open(_p2 + ".brief.checkpoint", "w").write("ck2")
...
_pout = _strip_dwm(_dw.run({"action": "pause", "pid": 6001}, str(FIX)))
check("dispatch_worker: pause kills + writes the suspended sentinel",
      "paused worker 6001" in _pout and 6001 in _killed_dw
      and _os_dw.path.exists(_p1 + ".brief.suspended") and _dw._H["workers"][6001].get("paused"))
...
check("dispatch_worker: pause_all scoped to a board pauses only that team",
      "pid 6002 paused" in _paout and 6002 in _killed_dw and 6001 not in _killed_dw)
...
# _board_sweep nukes an aged-out board but spares a fresh one
_bold, _bfresh = 111111, 222222
lc._board_claim(_bold, "x", "w:1"); lc._board_claim(_bfresh, "y", "w:1")
import os as _os_b, time as _time_b
_oldclaims = lc._board_claims_path(_bold)
_os_b.utime(_oldclaims, (_time_b.time() - 7200, _time_b.time() - 7200))  # age it 2h
lc._board_sweep(grace=3600)
check("board: sweep nukes an aged-out board dir", not lc._board_dir(_bold).exists())
check("board: sweep spares a fresh board dir", lc._board_dir(_bfresh).is_dir())
lc._board_teardown(_bfresh)
...
lc._bg_save(_dg_reg_save)                           # restore registry (drop the test worker rec)
...
lc._bg_save(_slw_reg_save)                         # restore registry (drop the test worker recs)
