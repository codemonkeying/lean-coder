...
One long flat script: ~1400 `check(name, cond)` assertions run top to bottom, then a
final ALL PASS / SOME FAILED. Ordering matters in places (see the note on section 0),
so tests are NOT freely reorderable. For navigation, major regions carry a unique
'# SECTION: <title>' banner (grep '^# SECTION:' for the map); the index below is
regenerated by tools/gen_section_index.py and drift-checked by the suite itself.

=== FILE MAP (regen: tools/gen_section_index.py) ===
  L68     Tool schemas, note tool, command/help contract
  L443    Core tool bodies (ignore, diff parser, read/write/apply, search, run_command)
  L2447   Config drift guard + queue-drain + host normalization
  L3466   Ollama num_ctx, host failover, per-host models, machines
  L3888   Remote executor + SSH transport + RemoteWorkspace + Windows
  L5316   Session lifecycle: autosave, snapshots, statusline
  L5648   Composer: input buffer, queue, paste/key parsing
  L6257   Version/update precedence + provider registration API
  L6932   Session modes + CLI arg completion + /set + leash + per-model
  L7419   Grouped lean-tools + provider/tool discovery dirs
  L7723   Per-process run dirs (liveness reaper, no age)
  L7753   Hygiene sweep + FILE MAP drift gate (final gates)
=== END FILE MAP ===
"""
...
# bg tasks live in per-process run dirs (no shared registry). Point run_dir at a scratch
# root for the WHOLE suite so tests never touch the user's real runtime dir, and give
# them a reset helper: "forget every task this process owns" (kill + remove its dirs).
_TEST_RUN_ROOT = Path(tempfile.mkdtemp(prefix="lc_run_"))
lc.run_dir = lambda: _TEST_RUN_ROOT
def _bg_reset():
    lc._bg_kill_session()
def _bg_restore(snapshot):
    """Drop records added since `snapshot` WITHOUT killing (some tests register a record
    whose pid is a real live process, e.g. os.getppid() - never signal those)."""
    keep = {r.get("log") for r in snapshot}
    for r in lc._bg_load():
        if r.get("log") not in keep:
            lc._bg_rec_path(r["log"]).unlink(missing_ok=True)
atexit.register(lambda: (lc._bg_kill_session(), shutil.rmtree(_TEST_RUN_ROOT, ignore_errors=True)))
...
# ==========================================================================
# SECTION: Tool schemas, note tool, command/help contract
# ==========================================================================
...
# 1. Context overhead limits. The README's overhead table (after the 'overhead-gate'
# marker) is the SINGLE source of truth: its two numbers are the pass/fail limits here,
# so the doc can't drift from reality. One token over fails - trim, or raise the README.
import re as _re
_rm = (Path(lc.__file__).parent / "README.md").read_text()
_gate = _rm.split("<!-- overhead-gate", 1)[1] if "<!-- overhead-gate" in _rm else ""
_lim = {k: int(v) for k, v in _re.findall(r"^\|\s*(Always-on|Every bundled)[^|]*\|\s*(\d+)\s*\|", _gate, _re.M)}
check("README overhead table present + parseable (Always-on, Every bundled)",
      set(_lim) == {"Always-on", "Every bundled"}, str(_lim))
_core_lim, _full_lim = _lim.get("Always-on", 0), _lim.get("Every bundled", 0)
sys_msg = {"role": "system", "content": lc.SYSTEM_PROMPT + "\nWorking directory: /x"}
overhead = lc.messages_tokens([sys_msg], lc.active_tools(lc.Config()))
check(f"always-on overhead <= README limit {_core_lim}", overhead <= _core_lim, f"~{overhead} tokens")
# ...and the FULL shipped surface (core + EVERY bundled lean-tool loaded).
_full_mgr = lc.LeanToolManager(str(Path(lc.__file__).parent / "lean-tools"), enabled=None)
full_overhead = lc.messages_tokens([sys_msg], lc.active_tools(lc.Config(), lean_tool_schemas=_full_mgr.schemas()))
check(f"full surface (all bundled lean-tools) <= README limit {_full_lim}",
      full_overhead <= _full_lim, f"~{full_overhead} tokens")
...
_rh.cfg = lc.Config(compact_soft=0.70, compact_at=0.95,
...
# /rewind: drop the last N user turns (the refusal/derail escape). Cuts at a user-message
# boundary so whole assistant+tool sequences go atomically - an orphaned tool_call with no
# matching tool result is a hard 400 on Anthropic/OpenAI, so pairing is the key invariant.
def _rw_convo():
...
def _rw_paired(msgs):
    _ids = [tc["id"] for m in msgs if m.get("role") == "assistant"
            for tc in (m.get("tool_calls") or [])]
    _res = [m.get("tool_call_id") for m in msgs if m.get("role") == "tool"]
    return sorted(_ids) == sorted(_res)

_rw = lc.Agent(lc.Config(cwd=FIX))
_rw.messages = _rw_convo()
check("/rewind: turn boundaries are the user messages", _rw.user_turn_indices() == [1, 5, 10])
_n, _m, _c, _last = _rw.rewind(1)
check("/rewind 1 drops one turn back to the prior assistant reply",
      (_n, _m, _c) == (1, 2, 0) and _last == "t3" and _rw.messages[-1]["content"] == "refused")
check("/rewind 1 keeps tool_call/tool_result pairing intact", _rw_paired(_rw.messages))
_rw.messages = _rw_convo()
_n, _m, _c, _last = _rw.rewind(2)
check("/rewind 2 drops whole assistant+tool spans atomically (counts tool calls)",
      (_n, _m, _c) == (2, 7, 2) and _last == "t2" and _rw.messages[-1]["content"] == "done1")
check("/rewind 2 keeps pairing intact", _rw_paired(_rw.messages))
_rw.messages = _rw_convo()
check("/rewind clamps past the start, keeping only the system prompt",
      _rw.rewind(99)[0] == 3 and len(_rw.messages) == 1
      and _rw.messages[0]["role"] == "system")
_rw.messages = _rw_convo()
check("/rewind 0 is a no-op", _rw.rewind(0) == (0, 0, 0, "") and len(_rw.messages) == 12)
_rw.messages = [{"role": "system", "content": "s"}]
check("/rewind on an empty conversation is a no-op", _rw.rewind(1) == (0, 0, 0, ""))
_rw.messages = _rw_convo()
_rw.last_prompt_tokens = 123
_rw.rewind(1)
check("/rewind clears the cached prompt-token count (meter re-estimates)",
      _rw.last_prompt_tokens is None)
# pinned plan + notebook are separate agent state and must survive a rewind
_rw.messages = _rw_convo()
_rw.pinned_plan = "GOAL: keep me"
_rw.rewind(2)
check("/rewind leaves the pinned plan alone", _rw.pinned_plan == "GOAL: keep me")
# handler: arg parsing (leading int = N, else the whole arg is the new prompt) + queueing
_rwh = lc.Agent(lc.Config(cwd=FIX))
_rwh.messages = _rw_convo()
with _ctx_note.redirect_stdout(_io_note.StringIO()):
    lc.handle_rewind_command(_rwh, _rwh.cfg, "1 explain it instead, no exploit code")
check("/rewind N <text> rewinds then queues the text as the next turn",
      _rwh._queued_turns == ["explain it instead, no exploit code"]
      and _rwh.messages[-1]["content"] == "refused")
_rwh.messages = _rw_convo()
_rwh._queued_turns = []
with _ctx_note.redirect_stdout(_io_note.StringIO()):
    lc.handle_rewind_command(_rwh, _rwh.cfg, "try again differently")
check("/rewind <text> with no leading int = 1 turn + that text",
      _rwh._queued_turns == ["try again differently"]
      and _rwh.messages[-1]["content"] == "refused")
_rwh.messages = _rw_convo()
_rwh._queued_turns = []
with _ctx_note.redirect_stdout(_io_note.StringIO()):
    lc.handle_rewind_command(_rwh, _rwh.cfg, "")
check("bare /rewind = one turn, nothing queued",
      _rwh._queued_turns == [] and _rwh.messages[-1]["content"] == "refused")
_rwout = _io_note.StringIO()
_rwh.messages = _rw_convo()
with _ctx_note.redirect_stdout(_rwout):
    lc.handle_rewind_command(_rwh, _rwh.cfg, "2")
check("/rewind warns that files on disk are NOT reverted when tool calls are dropped",
      "NOT reverted" in _rwout.getvalue())
_rwout2 = _io_note.StringIO()
_rwh.messages = [{"role": "system", "content": "s"}]
with _ctx_note.redirect_stdout(_rwout2):
    lc.handle_rewind_command(_rwh, _rwh.cfg, "1")
check("/rewind says so when there's nothing to rewind",
      "nothing to rewind" in _rwout2.getvalue())
check("/rewind is fully wired (dispatch+slash+help)",
      "/rewind" in lc._BUILTIN_COMMANDS_TABLE and "/rewind" in lc.SLASH_COMMANDS
      and any(c[0].startswith("/rewind") for c in lc.HELP_COMMANDS))
# argcomp: offer the turn depths that actually exist (honours the "Tab completes inline
# arguments" contract), capped, and degrading to [] on an agent that can't report.
_rwc = lc.Agent(lc.Config(cwd=FIX))
_rwc.messages = _rw_convo()
check("argcomp: /rewind offers the available turn counts",
      lc._arg_completions(_rwc, _rwc.cfg, "/rewind") == ["1", "2", "3"])
_rwc.messages = [{"role": "system", "content": "s"}]
check("argcomp: /rewind offers nothing on an empty conversation",
      lc._arg_completions(_rwc, _rwc.cfg, "/rewind") == [])
_rwc.messages = ([{"role": "system", "content": "s"}]
                 + [{"role": "user", "content": f"t{i}"} for i in range(30)])
check("argcomp: /rewind caps the offered depth",
      lc._arg_completions(_rwc, _rwc.cfg, "/rewind") == [str(i) for i in range(1, 10)])
check("argcomp: /rewind degrades to [] on an agent with no turn index",
      lc._arg_completions(object(), lc.Config(cwd=FIX), "/rewind") == [])
with _ctx_note.redirect_stdout(_io_note.StringIO()):
...
class _RC0:
    returncode = 0
_oe, _otc, _org = lc._edit_prompt, lc._tee_capture, lc._refresh_gpg_agent_tty
...
lc._refresh_gpg_agent_tty = lambda: _gpg_refreshed.update(n=_gpg_refreshed["n"] + 1) or dict(os.environ)
try:
    ...
    # the LOCAL ask_user_to_run path registers the gpg-agent TTY (so a GPG-subkey ssh
    # key can draw pinentry here); gated no-op for non-gpg users, but the CALL must fire.
    check("run_direct_command local path refreshes gpg-agent TTY", _gpg_refreshed["n"] == 1)
finally:
    lc._edit_prompt, lc._tee_capture, lc._refresh_gpg_agent_tty = _oe, _otc, _org
...
# ==========================================================================
# SECTION: Core tool bodies (ignore, diff parser, read/write/apply, search, run_command)
# ==========================================================================
...
# 3d. INDENTED markers parse - a model very commonly indents the marker line to match the
#     code it's editing; leading whitespace must be tolerated (was: "missing >>>>>>> REPLACE").
_ind_blocks, _ind_err = lc._parse_search_replace(
    "    <<<<<<< SEARCH\n    old\n    =======\n    new\n    >>>>>>> REPLACE")
check("indented SEARCH/REPLACE markers parse",
      _ind_err is None and _ind_blocks == [("    old", "    new")], f"{_ind_err} {_ind_blocks}")

# 4. truncation helper (moved into the bundled builtins module with run_command)
big = "\n".join(str(i) for i in range(5000))
trunc = lc._BUILTINS._truncate_output(big)
check("run_command output truncated", "omitted" in trunc and len(trunc) < len(big))
check("run_command truncation says HOW to see the middle (re-run narrower, no file kept)",
      "re-run narrower" in trunc and "| grep" in trunc and "read_file" in trunc)
check("operator-output clip says how to get the middle",
      "re-run it narrower" in lc._clip_output(big))
_seq = "\n".join(str(i) for i in range(1, 20001)) + "\n"
for _nm, _fn in (("run_command", lc._BUILTINS._truncate_output), ("operator clip", lc._clip_output)):
    _o = _fn(_seq); _h, _, _t = _o.partition("…[")
    _t = _t.split("]…\n", 1)[1]
    _hl = _h.rstrip("\n").splitlines()[-1]; _tl = _t.splitlines()[0]
    check(f"{_nm} truncation cuts on line boundaries (no fragment either side)",
          int(_tl) == int(_hl) + 1 + (int(_tl) - int(_hl) - 1)       # both are whole numbers
          and _seq.startswith(_h.rstrip("\n") + "\n") and _seq.endswith(_t), f"{_hl!r} / {_tl!r}")
...
check("command table: all expected commands present",
      {"/clear", "/new", "/connect", "/tools", "/reload", "/compact", "/trim",
       ...
       "/info", "/set", "/model", "/approve", "/prompt",
       "/sh"}.issubset(_tbl))
check("command table: /handover dropped (word reserved for future session-transfer)",
      "/handover" not in _tbl)
check("command table: /ctx removed (its ctx% line is already in /usage and /info)",
      "/ctx" not in _tbl)
...
check("autosave-after set names only settings-persisting commands",
      lc._CMD_AUTOSAVE == frozenset({"/provider", "/providers", "/set", "/compact_at"}) and lc._CMD_AUTOSAVE <= set(_tbl))
class _QuitAgent:
    messages = [{"role": "system", "content": "S"}]   # empty convo -> autosave no-ops
...
with _ctx_h.redirect_stdout(_io_h.StringIO()):
    _dctx = lc.dispatch_command(_da, _da.cfg, "/info", "")
...
# 4b. list_files / search_files caps name the exact narrowing call
_capdir = FIX / "capdir"
for _i in range(3):
    (_capdir / f"s{_i}").mkdir(parents=True, exist_ok=True)
    for _j in range(200):
        (_capdir / f"s{_i}" / f"f{_j}.txt").write_text("hitme\nhitme\n")
_capt = lc.Tools(lc.Config(cwd=_capdir))
_capl = _capt.list_files()
check("list_files cap names the follow-up (list_files path=<dir>)", "list_files path=<dir>" in _capl)
check("list_files cap names the dirs NOT yet listed (real paths, not a placeholder)",
      "not yet listed: s2;" in _capl, _capl.splitlines()[-1])   # s0 whole, s1 partly, s2 never reached
check("search_files cap names the follow-up (tighter pattern / path=)",
      "Narrow it: a tighter pattern, or path=" in _capt.search_files("hitme"))
_one = _capdir / "one.txt"; _one.write_text("hitme\n" * 500)
(_capdir / "lnk").mkdir(); (_capdir / "lnk" / "real").write_text("x")
os.symlink("real", _capdir / "lnk" / "alias")
os.symlink(str(_capdir / "s0"), _capdir / "lnk" / "dirlink")
_lk = lc.Tools(lc.Config(cwd=_capdir)).list_files("lnk")
check("list_files shows a symlink under its OWN name with its target (not the target twice)",
      "lnk/alias -> real" in _lk and _lk.count("lnk/real") == 1, _lk)
check("list_files never descends a symlinked dir",
      "dirlink/ -> " in _lk and "dirlink/f0.txt" not in _lk and "s0/f0" not in _lk, _lk)
check("search_files cap notice also fires for a SINGLE-file search (was silent)",
      "stopped at" in _capt.search_files("hitme", "one.txt"))
shutil.rmtree(_capdir, ignore_errors=True)
# diagnostics / ssh / git_summary: head+tail (errors + summaries live at the END) + how-to
import importlib.util as _ilu_t
def _load_lt(n):
    _sp = _ilu_t.spec_from_file_location(f"_lt_{n}", Path(lc.__file__).parent / "lean-tools" / f"{n}.py")
    _m = _ilu_t.module_from_spec(_sp); _sp.loader.exec_module(_m); return _m
_ssh_m = _load_lt("ssh")
class _R:
    def __init__(self, o): self.returncode, self.stdout, self.stderr = 1, o, "FATAL at the end"
_ssh_real = _ssh_m.subprocess.run
_ssh_m.subprocess.run = lambda *a, **k: _R("A" * 9000)
_so = _ssh_m.run({"host": "h", "cmd": "c"}, str(FIX))
_ssh_m.subprocess.run = _ssh_real
check("ssh cap keeps the TAIL (stderr/errors) + says how to re-run narrower",
      "FATAL at the end" in _so and "re-run narrower on the remote" in _so, _so[-200:])
_gs = _load_lt("git_summary")
_clipped = _gs._clip("status\n" + "\n".join(f" M f{i}" for i in range(400)), "status")
check("git_summary clipped section names the full command",
      "all of it: run_command `git status --short`" in _clipped)
# agent-run: a worker whose model only ever returns EMPTY (retries already spent by the
# provider) must not be nudged again, and the DRIVER gets a neutral NO RESPONSE result -
# the facts (retries, stop_reason) and possible causes, never a "refused" verdict. Run in a
# child process: it pins Agent.client, which must not leak into the rest of this suite.
_ew_code = r"""
import sys, os, io, contextlib
from pathlib import Path
sys.path.insert(0, sys.argv[1]); import lean_coder as lc
turns = [0]
class E:
    uncapped_ctx = True; last_out_tokens = 0; last_cache_read = 0; last_cache_write = 0
    last_stop_reason = "refusal"; last_empty_retries = 5
    def chat(self, m, t, should_abort=None):
        turns[0] += 1; return {"role": "assistant", "content": ""}, 10, False
    def detect_num_ctx(self): return 32768
    def list_models(self): return []
    def running_models(self): return []
_e = E(); lc.Agent.client = property(lambda s: _e, lambda s, v: None)
b = Path(sys.argv[2]); b.write_text(f"{lc.BRIEF_MARK}\nsay hi\n{lc.BRIEF_MARK}\n")
sys.argv = ["lc", "--agent-run", "--brief-file", str(b), "--cwd", str(b.parent)]
with contextlib.redirect_stdout(io.StringIO()):
    try: lc.main()
    except SystemExit: pass
print(turns[0]); print(Path(lc._brief_result_path(str(b), None)).read_text().strip().splitlines()[1])
"""
_ewd = Path(tempfile.mkdtemp(prefix="lc_ew_"))
_ewr = subprocess.run([sys.executable, "-c", _ew_code, str(Path(lc.__file__).parent), str(_ewd / "b.brief")],
                      capture_output=True, text=True, timeout=120,
                      env={**os.environ, "LEANCODER_CONFIG_DIR": str(_ewd / "cfg")})
_ewo = _ewr.stdout.strip().splitlines()
check("agent-run: exhausted empty reply -> no rescue nudges (exactly 1 model turn)",
      _ewo[:1] == ["1"], _ewr.stdout[-300:] + _ewr.stderr[-300:])
check("agent-run: driver gets a neutral NO RESPONSE result (facts + possible causes, no verdict)",
      len(_ewo) > 1 and _ewo[1].startswith("NO RESPONSE: the model returned an empty reply after 5 "
                                           "retries (last stop_reason: refusal). Possible causes:"),
      _ewo[1:2])
shutil.rmtree(_ewd, ignore_errors=True)

# /push + /pull: what this box offers (user plugins + prompts + provider logins, never a
# bundled plugin), the same-hash skip, and the headless --enable config edit.
_sy = Path(tempfile.mkdtemp(prefix="lc_sync_"))
for _d in ("providers", "lean-tools", "prompts/system"):
    (_sy / _d).mkdir(parents=True, exist_ok=True)
(_sy / "providers/myprov.py").write_text("PROVIDER = {}\n")
(_sy / "lean-tools/mytool.py").write_text("TOOL = {}\n")
(_sy / "prompts/reviewer.txt").write_text("r\n")
(_sy / "prompts/system/system.txt").write_text("s\n")
(_sy / "auth-work.json").write_text('{"account": {"email_address": "w@example.test"}}')
_sv = (lc.CONFIG_DIR, lc.PROMPTS_DIR, lc.SYSTEM_PROMPTS_DIR)
lc.CONFIG_DIR, lc.PROMPTS_DIR, lc.SYSTEM_PROMPTS_DIR = _sy, _sy / "prompts", _sy / "prompts/system"
_syc = lc.Config(model="m"); _syc.providers_dir = str(_sy / "providers"); _syc.lean_tools_dir = str(_sy / "lean-tools")
_syi = lc._sync_local_items(_syc)
_synames = {k: [r["name"] for r in v] for k, v in _syi.items()}
check("sync: offers the user's own provider/tool/prompts + a login under its provider",
      "myprov" in _synames.get("providers", []) and "login: work" in _synames["providers"]
      and _synames.get("lean-tools") == ["mytool"]
      and set(_synames.get("prompts", [])) == {"reviewer", "system (built-in override)"}, _synames)
check("sync: never offers a bundled plugin (the code dir shadows a pushed copy)",
      not any(n in ("ollama", "groq", "ssh", "notify", "word_count")
              for v in _synames.values() for n in v), _synames)
check("sync: a login belongs to the anthropic_plan provider (not a kind of its own)",
      [r["parent"] for r in _syi["providers"] if r["name"] == "login: work"] == ["anthropic_plan"])
check("sync: status - same hash skips, a missing target is new, else differs",
      lc._sync_status("a", "a") == "same" and lc._sync_status("a", None) == "new"
      and lc._sync_status("a", "b") == "differs")
lc.CONFIG_DIR, lc.PROMPTS_DIR, lc.SYSTEM_PROMPTS_DIR = _sv
# --enable: a headless config edit, refusing names with no plugin file
_eh = Path(tempfile.mkdtemp(prefix="lc_en_"))
_er = subprocess.run([sys.executable, str(Path(lc.__file__)), "--enable", "tool:word_count",
                      "--enable", "provider:groq", "--enable", "tool:nope"],
                     capture_output=True, text=True, timeout=60, env={**os.environ, "HOME": str(_eh)})
_etoml = (_eh / ".config/leancoder/config.toml").read_text() if (_eh / ".config/leancoder/config.toml").is_file() else ""
check("--enable: adds provider/tool names to the saved config",
      '"word_count"' in _etoml and '"groq"' in _etoml, _er.stdout + _er.stderr)
check("--enable: refuses a name with no plugin file (exit 2, not enabled)",
      _er.returncode == 2 and "no tool named 'nope'" in _er.stdout and '"nope"' not in _etoml)
shutil.rmtree(_sy, ignore_errors=True); shutil.rmtree(_eh, ignore_errors=True)

# 5. read_file line numbering + truncation notice
(FIX / "big.txt").write_text("\n".join(f"line{i}" for i in range(2000)))
r = tools.read_file("big.txt")
check("read_file numbers + truncates with the exact follow-up call",
      "\t" in r and "lines 451-1900 omitted; read them: read_file start=451 end=1050" in r, r[-300:])
check("read_file big-file notice also says how to JUMP (search_files for a line, then a range)",
      "or jump: search_files path=" in r and "then read_file start=/end= around it" in r)
check("read_file range cap names the next start",
      "more: read_file start=1051" in tools.read_file("big.txt", start=451, end=1900))
...
# 7e. invisible-unicode scrubber: model-supplied text is stripped of zero-width /
# format-control chars before it hits disk (defeats fingerprint/watermark smuggling),
# while legit visible unicode + tabs/newlines survive. Opt-out via scrub_invisible.
_wf_msg = tools.write_file("scrub.py", "x = 1\u200b\n# c\u200dafe \u2615\ufeff\n")
check("scrub: write_file strips invisibles", "invisible char" in _wf_msg)
_wrote = (FIX / "scrub.py").read_text()
check("scrub: no zero-width/BOM left on disk",
      not any(ord(c) in {0x200b, 0x200d, 0xfeff} for c in _wrote))
check("scrub: visible unicode preserved", "\u2615" in _wrote and "cafe" in _wrote)
check("scrub: tabs/newlines untouched", _wrote.count("\n") == 2)
# opt-out writes bytes through untouched
_cfg_noscrub = lc.Config(cwd=FIX, approval="auto", scrub_invisible=False)
_ns_tools = lc.Tools(_cfg_noscrub)
_ns_tools.write_file("noscrub.py", "y = 2\u200b\n")
check("scrub: opt-out leaves invisibles in place",
      "\u200b" in (FIX / "noscrub.py").read_text())
...
check("list_files outside cwd yields real absolute paths (no mangle)",
      all(l.rstrip("/").startswith(str(_ext)) for l in _extlst.splitlines() if "…[" not in l)
...
check("run_command: background writes its log in this process's run dir",
      str(lc._run_pid_dir()) in bgr and lc._run_pid_dir().is_dir())
...
# Spinner width-clip: a long-running watchdog label (elapsed + '^C to stop (kill ...)')
# must be clipped to the terminal width so it can't wrap onto a 2nd physical line on a
# narrow screen (mobile/Termux) - an unclipped wrap made '\r' spam a new line every
# repaint on a slow push. _visible_len of the clipped line stays within the width.
_wd_lbl = "◐ " + lc._activity_label("pushing agent", 40, 1197617)  # ~70+ chars
_clipped = lc._fit_line(_wd_lbl, 44)   # a narrow portrait Termux width
check("spinner: long watchdog label clips to width (no wrap -> no newline spam)",
      len(lc._ANSI_RE.sub("", _clipped)) <= 44 and len(lc._ANSI_RE.sub("", _wd_lbl)) > 44)
...
_bg_reset()                                        # clean registry for the status subtests
...
# pending-bg: _own_live_bg_jobs backs the worker-loop park (a worker must not exit while
# its own bg job is live - on a remote executor, exiting kills the executor + the job).
# It's plain-task only (a worker's child workers notify via their own path) and never raises.
check("_own_live_bg_jobs: no jobs for a foreign owner -> empty (safe to finish)",
      lc._own_live_bg_jobs(owner=999999) == [])
_olbj_orig = lc._bg_running
lc._bg_running = lambda owner=None, kind="task": ([{"pid": 1, "kind": "task"}] if kind == "task" else [])
check("_own_live_bg_jobs: a live plain task blocks finish", len(lc._own_live_bg_jobs(owner=1)) == 1)
lc._bg_running = lambda owner=None, kind="task": []
check("_own_live_bg_jobs: no live task -> safe to finish", lc._own_live_bg_jobs(owner=1) == [])
lc._bg_running = _olbj_orig
...
lc._bg_kill_session()
_bg_reset()                                        # drop the fake foreign-pid worker record too
...
_bg_reset()
_afklog = str(lc._run_pid_dir() / "afk-worker.log")
...
# owner-graph cascade: reaches setsid-escaped grandchildren a single killpg would miss.
_casc = [
    {"pid": 200, "owner": 100, "host": lc._HOSTNAME},   # worker (owned by driver 100)
    {"pid": 300, "owner": 200, "host": lc._HOSTNAME},   # its worker (grandchild)
    {"pid": 400, "owner": 300, "host": lc._HOSTNAME},   # great-grandchild
    {"pid": 999, "owner": 500, "host": lc._HOSTNAME},   # unrelated subtree
    {"pid": 777, "owner": 200, "host": "otherbox"},     # foreign host -> never ours to kill
]
check("bg: descendant cascade is leaves-first, foreign-host excluded",
      lc._bg_descendant_pids(200, _casc) == [400, 300])
check("bg: descendant cascade from the driver reaches the whole subtree",
      lc._bg_descendant_pids(100, _casc) == [400, 300, 200])
check("bg: descendant cascade of a leaf is empty", lc._bg_descendant_pids(400, _casc) == [])
# cycle-safe (a corrupt registry can't infinite-loop the walk)
_cyc = [{"pid": 1, "owner": 2, "host": lc._HOSTNAME}, {"pid": 2, "owner": 1, "host": lc._HOSTNAME}]
check("bg: descendant cascade is cycle-safe", set(lc._bg_descendant_pids(1, _cyc)) <= {1, 2})
check("bg: _bg_kill_tree never raises on a dead/absent pid",
      lc._bg_kill_tree(2 ** 31 - 1) is None)
...
# --- board lean-tool: MUST be driver_only -------------------------------------
# The board is driver-orchestration state (a JSON doc under the DRIVER's config dir that
# reconcile + peer sessions read from there). Without driver_only a /connect routes it to
# the remote --tool-exec executor, which never runs setup() -> the tool's core hooks are
# unset -> "board tool is not initialised" (a live bug we hit this way), and it would also
# write the JSON to the wrong box. Pin it to the driver.
_bd_path = Path(__file__).resolve().parent.parent / "lean-tools" / "board.py"
if _bd_path.is_file():
    _bd = lc._load_lean_tool(_bd_path)
    check("board: is a driver_only tool (runs on the driver, not a /connect remote)",
          _bd.TOOL.get("driver_only") is True)

# --- /board slash command: read-only board inspection WITHOUT enabling the tool -------
# A board is a JSON doc under the config dir, so the operator can inspect it directly - no
# ~500-token tool schema in context, no model turn. Wired (dispatch/slash/help), lists +
# shows + argcomps board names, and handles the not-found path.
check("/board is fully wired (dispatch+slash+help)",
      "/board" in lc._BUILTIN_COMMANDS_TABLE and "/board" in lc.SLASH_COMMANDS
      and any(c[0].startswith("/board") for c in lc.HELP_COMMANDS))
import io as _bio, contextlib as _bctx
_bd_name = "smoketest_board_xyz"
_bd_real_tb = lc.TASKBOARDS_DIR
lc.TASKBOARDS_DIR = Path(tempfile.mkdtemp(prefix="lc_tb_"))   # never touch the real boards
_bd_p = lc._taskboard_path(_bd_name)
if _bd_p and _bd_p.is_file():
    _bd_p.unlink()
lc._taskboard_create(_bd_name, "T", "drv")
lc._taskboard_mutate(_bd_name, lambda bd: lc._taskboard_add(bd, "alpha", deps=[]))
lc._taskboard_mutate(_bd_name, lambda bd: lc._taskboard_add(bd, "beta", deps=["t1"]))
_bout = _bio.StringIO()
with _bctx.redirect_stdout(_bout):
...
check("/board (no arg) lists the board by name", _bd_name in _lout.getvalue())
check("argcomp: /board completes the delete verb + board names",
      "delete" in lc._arg_completions(object(), lc.Config(), "/board")
      and _bd_name in lc._arg_completions(object(), lc.Config(), "/board"))
_nout = _bio.StringIO()
with _bctx.redirect_stdout(_nout):
    lc.handle_board_command(object(), lc.Config(), "definitely_no_such_board")
check("/board <missing> reports not-found (never raises)",
      "no board named" in _nout.getvalue())
_bd_p.unlink()   # cleanup the throwaway board
shutil.rmtree(lc.TASKBOARDS_DIR, ignore_errors=True)
lc.TASKBOARDS_DIR = _bd_real_tb
# claims board: lives in the DRIVER's run dir (per-process scratch), keyed by its pid
check("claims board: lives under the driver's run dir",
      lc._board_dir(lc.os.getpid()) == lc._run_pid_dir() / "board")
_cb_ok, _ = lc._board_claim(lc.os.getpid(), "a.py", "w1")
check("claims board: claim works in the run dir",
      _cb_ok and lc._board_claims_path(lc.os.getpid()).is_file())
lc._board_teardown(lc.os.getpid())
check("claims board: teardown removes it", not lc._board_dir(lc.os.getpid()).exists())
# legacy CONFIG_DIR/workers/board/<pid>: dead driver's removed, a live one's kept
_lb = lc.CONFIG_DIR / "workers" / "board"
_lbdead = subprocess.Popen(["true"]); _lbdead.wait()
(_lb / str(_lbdead.pid)).mkdir(parents=True, exist_ok=True)
(_lb / str(_lbdead.pid) / "claims.jsonl").write_text("")
(_lb / str(lc.os.getpid())).mkdir(parents=True, exist_ok=True)
lc._board_sweep()
check("legacy claims boards: dead driver's removed, live one's kept (no age rule)",
      not (_lb / str(_lbdead.pid)).exists() and (_lb / str(lc.os.getpid())).exists())
...
_os_dw.environ.pop("LEANCODER_WORKER_MODELS", None)
# action='models' lists dispatchable models per provider (a read; no worker spawned)
import re as _re_dwm
_strip_dwm = lambda s: _re_dwm.sub(r"\x1b\[[0-9;]*m", "", s or "")
_dw._H["enabled_provider_models"] = lambda: {"anthropic_plan": ["big-model", "cheap-leaf"]}
_mout = _strip_dwm(_dw.run({"action": "models"}, str(FIX)))
check("dispatch_worker: models lists provider + its models",
      "anthropic_plan" in _mout and "cheap-leaf" in _mout)
_os_dw.environ["LEANCODER_WORKER_MODELS"] = "cheap-leaf"
_mout2 = _strip_dwm(_dw.run({"action": "models"}, str(FIX)))
check("dispatch_worker: models honours the operator allowlist",
      "cheap-leaf" in _mout2 and "big-model" not in _mout2)
_os_dw.environ.pop("LEANCODER_WORKER_MODELS", None)
# action='transcript' renders a worker's checkpointed reasoning/tool trail, paged
import json as _json_dwt
_dwt_stamp = str(FIX / "wk_tr")
_dwt_ckpt = _dwt_stamp + ".brief.checkpoint"   # _brief_from_result(<stamp>.result)+.checkpoint
open(_dwt_stamp + ".result", "w").close()
_json_dwt.dump({"messages": [
    {"role": "system", "content": "SYS"},
    {"role": "user", "content": "long worker preamble boilerplate here\nTASK:\ndo the thing"},
...
_dw._H["workers"][4242] = {"task": "transcript test", "result": _dwt_stamp + ".result"}
_tout = _strip_dwm(_dw.run({"action": "transcript", "pid": 4242}, str(FIX)))
check("dispatch_worker: transcript renders worker turns (no system)",
      "do the thing" in _tout and "read_file" in _tout and "file body" in _tout and "SYS" not in _tout)
check("dispatch_worker: transcript collapses the preamble to the task",
      "TASK: do the thing" in _tout and "boilerplate" not in _tout)
check("dispatch_worker: transcript reports page count", "page 1/1" in _tout)
_dw._H["workers"][4243] = {"task": "no ckpt", "result": str(FIX / "wk_none.result")}
check("dispatch_worker: transcript without checkpoint explains",
      "no transcript checkpoint" in _strip_dwm(_dw.run({"action": "transcript", "pid": 4243}, str(FIX))))
del _dw._H["workers"][4242]
del _dw._H["workers"][4243]
# --- coverage for the remaining actions: status / result / cancel / inject ---
_RM = _dw._H["RESULT_MARK"]
# a FINISHED worker (result file carries a RESULT block) and a PENDING one (empty file)
_fin_res = str(FIX / "wk_fin.result")
open(_fin_res, "w").write(f"{_RM}\nall done: wrote the file\n{_RM}\n")
_pend_res = str(FIX / "wk_pend.result")
open(_pend_res, "w").close()
_dw._H["workers"][5001] = {"task": "finished worker task", "result": _fin_res,
                           "model": "haiku"}
_dw._H["workers"][5002] = {"task": "pending worker task", "result": _pend_res,
                           "model": "haiku"}
# action='status'
_st_all = _strip_dwm(_dw.run({"action": "status"}, str(FIX)))
check("dispatch_worker: status lists workers with ready/pending",
      "pid 5001" in _st_all and "result ready" in _st_all
      and "pid 5002" in _st_all and "result pending" in _st_all)
check("dispatch_worker: status narrows to one pid",
      "5001" in _strip_dwm(_dw.run({"action": "status", "pid": 5001}, str(FIX)))
      and "5002" not in _strip_dwm(_dw.run({"action": "status", "pid": 5001}, str(FIX))))
check("dispatch_worker: status unknown pid explains",
      "no worker with pid 9999" in _strip_dwm(_dw.run({"action": "status", "pid": 9999}, str(FIX))))
# action='result'
check("dispatch_worker: result returns the finished RESULT block",
      "all done: wrote the file" in _strip_dwm(_dw.run({"action": "result", "pid": 5001}, str(FIX))))
check("dispatch_worker: result on a pending worker says none yet",
      "no result yet" in _strip_dwm(_dw.run({"action": "result", "pid": 5002}, str(FIX))))
# action='cancel' (no-op on a finished worker; error paths without touching real processes)
check("dispatch_worker: cancel on a finished worker is a no-op",
      "already finished" in _strip_dwm(_dw.run({"action": "cancel", "pid": 5001}, str(FIX))))
check("dispatch_worker: cancel needs a pid",
      "needs a pid" in _strip_dwm(_dw.run({"action": "cancel"}, str(FIX))))
check("dispatch_worker: cancel unknown pid explains",
      "no worker with pid 9999" in _strip_dwm(_dw.run({"action": "cancel", "pid": 9999}, str(FIX))))
# action='inject' (writes to the pending worker's sidecar; finished worker rejects)
_inj_out = _strip_dwm(_dw.run({"action": "inject", "pid": 5002, "text": "focus on X"}, str(FIX)))
_inj_side = _dw._brief_from_result(_pend_res) + ".inject"
check("dispatch_worker: inject writes the sidecar for a running worker",
      "injected into worker 5002" in _inj_out
      and _os_dw.path.exists(_inj_side) and "focus on X" in open(_inj_side).read())
check("dispatch_worker: inject rejects an empty message",
      "non-empty text" in _strip_dwm(_dw.run({"action": "inject", "pid": 5002, "text": "  "}, str(FIX))))
check("dispatch_worker: inject too late on a finished worker",
...
# --- team lifecycle: pause / stop_all / pause_all ---
# the whole worker family is scratch now (no parked sentinel): one call wipes it
_susp_stamp = str(FIX / "wk_susp")
for _suf in (".brief", ".brief.checkpoint", ".brief.progress.silent"):
    open(_susp_stamp + _suf, "w").write("x")
lc._clean_worker_sidecars(_susp_stamp + ".brief")
check("worker cleanup: whole family removed (incl checkpoint + .progress.silent)",
      not any(_os_dw.path.exists(_susp_stamp + _suf)
              for _suf in (".brief", ".brief.checkpoint", ".brief.progress.silent")))
# pause (dispatch_worker): kill hook stubbed, checkpoint present, running row present
_killed_dw = []
_dw._H["_bg_kill"] = lambda p: _killed_dw.append(p)
_dw._H.pop("_bg_kill_tree", None)
_dw_bg_real = _dw._H.get("bg_status")
_dw._H["bg_status"] = lambda owner, kind=None: [{"pid": 6001, "state": "running", "runtime": "1s"},
                                                {"pid": 6002, "state": "running", "runtime": "1s"}]
_p1 = str(FIX / "wk_p1"); _p2 = str(FIX / "wk_p2")
_ck_env = lc.json.dumps({"meta": {"pinned_plan": "GOAL: farm"},
                         "messages": [{"role": "user", "content": "farm it"},
                                      {"role": "assistant", "content": "on it"}]})
open(_p1 + ".brief.checkpoint", "w").write(_ck_env)
open(_p2 + ".brief.checkpoint", "w").write(_ck_env)
open(_p1 + ".result", "w").close(); open(_p2 + ".result", "w").close()
_dw._H["workers"][6001] = {"task": "farm t", "result": _p1 + ".result", "brief": _p1 + ".brief",
                           "checkpoint": True, "taskboard": "farm"}
_dw._H["workers"][6002] = {"task": "mine t", "result": _p2 + ".result", "brief": _p2 + ".brief",
                           "checkpoint": True, "taskboard": "mine"}
_real_sess_dir = lc.SESSIONS_DIR
lc.SESSIONS_DIR = Path(tempfile.mkdtemp(prefix="lc_pause_"))   # never write the real sessions/
_pout = _strip_dwm(_dw.run({"action": "pause", "pid": 6001, "name": "farmer"}, str(FIX)))
_psess = lc._session_path("farmer")
check("dispatch_worker: pause saves a session (no sentinel) then kills",
      "saved as session 'farmer'" in _pout and 6001 in _killed_dw and _psess.exists()
      and not _os_dw.path.exists(_p1 + ".brief.suspended")
      and _dw._H["workers"][6001].get("paused"), _pout)
check("dispatch_worker: pause return names BOTH ways back (resume pid / from_session)",
      "action='resume' pid=6001" in _pout and "from_session='farmer'" in _pout)
_psdata = lc.json.loads(_psess.read_text())
check("dispatch_worker: paused session carries the transcript + plan",
      len(_psdata.get("messages", [])) >= 2
      and _psdata.get("meta", {}).get("pinned_plan") == "GOAL: farm", str(_psdata)[:200])
_psess.unlink(missing_ok=True)
_dw._H["workers"][6003] = {"task": "nock", "result": str(FIX / "wk_p3.result"),
                           "brief": str(FIX / "wk_p3.brief"), "checkpoint": False, "taskboard": ""}
open(str(FIX / "wk_p3.result"), "w").close()
check("dispatch_worker: pause refuses a non-checkpoint worker",
      "checkpointing off" in _strip_dwm(_dw.run({"action": "pause", "pid": 6003}, str(FIX))))
# pause_all scoped to a board pauses only that team (6002 on 'mine'; 6001 already paused)
_killed_dw.clear()
_paout = _strip_dwm(_dw.run({"action": "pause_all", "taskboard": "mine"}, str(FIX)))
check("dispatch_worker: pause_all scoped to a board pauses only that team",
      "pid 6002 -> session 'worker-6002'" in _paout and 6002 in _killed_dw
      and 6001 not in _killed_dw, _paout)
shutil.rmtree(lc.SESSIONS_DIR, ignore_errors=True)
lc.SESSIONS_DIR = _real_sess_dir
del _dw._H["workers"][6001]; del _dw._H["workers"][6002]; del _dw._H["workers"][6003]
if _dw_bg_real is not None:
    _dw._H["bg_status"] = _dw_bg_real
...
# SAFE-RECURSION governor: depth x child-budget gate on a worker spawning workers.
import os as _os_gov
for _v in ("LEANCODER_WORKER_MAX_DEPTH", "LEANCODER_WORKER_MAX_CHILDREN"):
    _os_gov.environ.pop(_v, None)
_dwcfg.worker_max_depth = 1
_dwcfg.worker_max_children = 0
# the driver (depth 0) may always spawn; its child is depth 1 with the max_children budget
_ok, _why, _bud = _dw._governor_check(0)
check("governor: driver may spawn (depth 0)", _ok and _bud == 0)
# a depth-1 worker with default max_depth 1 is blocked (no recursion by default)
_ok, _why, _bud = _dw._governor_check(1)
check("governor: depth-1 worker blocked at max_depth 1", (not _ok) and "depth limit" in _why)
# raise the ceiling but grant no child budget -> still blocked (opt-in on BOTH axes)
_dwcfg.worker_max_depth = 3
_dwcfg.worker_child_budget = 0
_ok, _why, _bud = _dw._governor_check(1)
check("governor: depth ok but zero child budget blocks", (not _ok) and "child budget" in _why)
# grant a budget of 2 -> spawn allowed, budget decrements, child inherits the remainder
_dwcfg.worker_child_budget = 2
_ok, _why, _bud = _dw._governor_check(1)
check("governor: budgeted worker may spawn; child inherits shrinking budget",
      _ok and _bud == 1 and _dwcfg.worker_child_budget == 1)
_ok2, _why2, _bud2 = _dw._governor_check(1)
check("governor: second spawn spends last budget (child budget 0 = leaf)",
      _ok2 and _bud2 == 0 and _dwcfg.worker_child_budget == 0)
_ok3, _why3, _bud3 = _dw._governor_check(1)
check("governor: budget exhausted -> refused", (not _ok3) and "child budget" in _why3)
# env var is a hard ceiling over cfg
_os_gov.environ["LEANCODER_WORKER_MAX_DEPTH"] = "1"
_dwcfg.worker_child_budget = 5
check("governor: env LEANCODER_WORKER_MAX_DEPTH overrides cfg (blocks depth 1)",
      not _dw._governor_check(1)[0])
_os_gov.environ.pop("LEANCODER_WORKER_MAX_DEPTH", None)
# depth + child_budget ride the GRANT block so a worker learns its place in the tree
_gbrief = _dw._compose_brief("t", "", "/tmp", 10, depth=2, child_budget=3)
check("governor: depth flows into GRANT block", "depth: 2\n" in _gbrief)
check("governor: child_budget flows into GRANT block", "child_budget: 3\n" in _gbrief)
_grant = lc._parse_grant(lc._extract_marked(_gbrief, lc.GRANT_MARK))
check("governor: worker parses depth + child_budget from grant",
      _grant.get("depth") == "2" and _grant.get("child_budget") == "3")
# default fan-out brief carries NO depth/budget lines (a plain worker, unchanged)
_pbrief = _dw._compose_brief("t", "", "/tmp", 10)
check("governor: default brief has no depth/child_budget (safe default)",
      "depth:" not in _pbrief and "child_budget:" not in _pbrief)
_dwcfg.worker_depth = 0
_dwcfg.worker_child_budget = 0
_dwcfg.worker_max_depth = 1
_dwcfg.worker_max_children = 0
# worker token spend is pure OBSERVABILITY: metered from finished-worker .usage sidecars,
# surfaced in /worker status. Nothing gates on it (the enforcement budget was removed as a
# leaky half-measure - concurrency + lease + the depth/child governor are the real guards).
_tbtmp = Path(tempfile.mkdtemp(prefix="lc_wspend_"))
_tbres = _tbtmp / "wt.result"
(_tbtmp / "wt.brief.usage").write_text("in=1200 out=800\n")
check("worker-spend: _read_usage parses the .usage sidecar",
      _dw._read_usage(str(_tbres)) == (1200, 800))
check("worker-spend: _read_usage of a missing sidecar -> (0,0)",
      _dw._read_usage(str(_tbtmp / "nope.result")) == (0, 0))
_dw._H["tokens_in"] = 0; _dw._H["tokens_out"] = 0
_tbmeta = {"result": str(_tbres), "task": "t", "model": "m", "leash": "r"}
_dw._accrue_usage(_tbmeta)
check("worker-spend: _accrue_usage adds to session totals",
      _dw._worker_tokens_spent() == 2000 and _tbmeta.get("usage_counted"))
_dw._accrue_usage(_tbmeta)   # idempotent - a re-scan must not double-count
check("worker-spend: _accrue_usage is idempotent (no double count)",
      _dw._worker_tokens_spent() == 2000)
_dw._H["tokens_in"] = 0; _dw._H["tokens_out"] = 0
_dw._H["workers"] = {}
# seed STATE blocks (context/plan/notes) emit their own marker blocks the worker parses
_seedbrief = _dw._compose_brief("do it", "", "/tmp", 10, tools="read_file",
                                context="auth refactored", plan="GOAL: x\n- [ ] step",
                                notes="found A\nfound B")
check("dispatch_worker: tools grant flows into GRANT block", "tools: read_file\n" in _seedbrief)
check("dispatch_worker: seed context -> CONTEXT block",
      lc._extract_marked(_seedbrief, lc.SEED_CONTEXT_MARK) == "auth refactored")
check("dispatch_worker: seed plan -> PLAN block",
      "step" in lc._extract_marked(_seedbrief, lc.SEED_PLAN_MARK))
check("dispatch_worker: seed notes -> NOTES block",
      lc._extract_marked(_seedbrief, lc.SEED_NOTES_MARK) == "found A\nfound B")
check("dispatch_worker: brief still parses with seeds present",
      lc._extract_marked(_seedbrief, lc.BRIEF_MARK) == "do it")
# the worker side must be able to read the tools allowlist back out of the grant (the
# parse that turns "tools: a,b" into cfg.tool_allowlist - a regression here silently gave
# every worker the full surface).
_gtools = lc._parse_grant(lc._extract_marked(_seedbrief, lc.GRANT_MARK))
check("dispatch_worker: worker parses tools allowlist back from grant",
      _gtools.get("tools") == "read_file")
# no seeds -> no seed blocks (everyday worker unchanged)
_bare = _dw._compose_brief("t", "", "/tmp", 10)
check("dispatch_worker: no seeds -> no seed marker blocks",
      lc.SEED_CONTEXT_MARK not in _bare and lc.SEED_PLAN_MARK not in _bare
      and lc.SEED_NOTES_MARK not in _bare)
# dispatch-time iterations cap: the driver may request a per-worker max_iterations, capped
# at the operator ceiling (min(requested, ceiling); junk/absent -> ceiling). Drive a real
# run() dispatch with bg_launch stubbed to capture the composed brief, then parse the GRANT
# back and confirm the clamp. Ceiling here is the default 30 (no env, fresh cfg).
_di_saved_launch = _dw._H["bg_launch"]
_di_saved_list = _dw._H.get("bg_list")
_di_cap = {}
def _di_launch(cmd, **kw):
    _di_cap["brief"] = kw.get("heartbeat_file", "").replace(".progress", "")
    return {"pid": 5150, "log": "/tmp/di.log"}
_dw._H["bg_launch"] = _di_launch
_dw._H["bg_list"] = lambda kind=None: []
def _di_dispatch_iter(req):
    _di_cap.clear()
    _dw.run({"task": "t", "iterations": req}, str(FIX))
    _g = lc._parse_grant(lc._extract_marked(Path(_di_cap["brief"]).read_text(), lc.GRANT_MARK))
    return int(_g.get("max_iterations"))
check("dispatch iterations: request below ceiling is honored",
      _di_dispatch_iter(7) == 7)
check("dispatch iterations: request above ceiling clamps to ceiling (30)",
      _di_dispatch_iter(999) == 30)
check("dispatch iterations: junk request falls back to ceiling (never crashes)",
      _di_dispatch_iter("abc") == 30)
check("dispatch iterations: non-positive request falls back to ceiling",
      _di_dispatch_iter(0) == 30 and _di_dispatch_iter(-4) == 30)
# a clamped request must SAY so in the return (silent clamp was the trap that made
# workers mysteriously die at 30); an in-budget request must not carry the note.
_di_cap.clear()
_di_over = _dw.run({"task": "t", "iterations": 999}, str(FIX))
check("dispatch iterations: over-ceiling request reports the clamp in the return",
      "capped to the 30 ceiling" in _di_over)
_di_cap.clear()
_di_ok = _dw.run({"task": "t", "iterations": 7}, str(FIX))
check("dispatch iterations: in-budget request carries no clamp note",
      "capped to the 30 ceiling" not in _di_ok)
_di_cap.clear()
_dw.run({"task": "t"}, str(FIX))   # omitted -> ceiling
check("dispatch iterations: omitted -> ceiling (30)",
      lc._parse_grant(lc._extract_marked(Path(_di_cap["brief"]).read_text(),
                                         lc.GRANT_MARK)).get("max_iterations") == "30")
check("dispatch: worker brief lives in the driver's run dir (shares its lifecycle)",
      Path(_di_cap["brief"]).parent == lc._run_pid_dir(), _di_cap["brief"])
_dw._H["bg_launch"] = _di_saved_launch
if _di_saved_list is not None:
    _dw._H["bg_list"] = _di_saved_list
_dw._H["workers"] = {}
# Phase 0 step 4: live steer sidecars (.brief.plan / .brief.note) MUST be registered in
# _WORKER_SIDECAR_SUFFIXES or they leak past cleanup.
check("live-steer: .brief.plan registered for cleanup",
      ".brief.plan" in lc._WORKER_SIDECAR_SUFFIXES)
check("live-steer: .brief.note registered for cleanup",
      ".brief.note" in lc._WORKER_SIDECAR_SUFFIXES)
# set_plan/add_note write their sidecar for a RUNNING worker, and refuse a finished one.
_lstmp = Path(tempfile.mkdtemp(prefix="lc_lsteer_"))
_lsres = _lstmp / "w9.result"
_lsbrief = _lstmp / "w9.brief"
_lsbrief.write_text("x")   # brief exists; result absent -> worker counts as running
_dw._H["workers"] = {909: {"result": str(_lsres), "task": "t", "model": "m", "leash": "r"}}
_saved_rows = _dw._worker_rows
_dw._worker_rows = lambda: {909: {"pid": 909, "state": "running", "runtime": "1s"}}
_r = _dw._worker_set_plan(909, "GOAL: new\n- [ ] a")
check("live-steer: set_plan queues for running worker", "queued a plan update" in _r)
check("live-steer: set_plan writes .brief.note-sibling .plan sidecar",
      Path(str(_lsbrief) + ".plan").exists()
      and "GOAL: new" in Path(str(_lsbrief) + ".plan").read_text())
_r = _dw._worker_add_note(909, "watch the rate limit")
check("live-steer: add_note queues for running worker", "queued a note" in _r)
check("live-steer: add_note writes .note sidecar",
      Path(str(_lsbrief) + ".note").exists()
      and "watch the rate limit" in Path(str(_lsbrief) + ".note").read_text())
check("live-steer: add_note refuses empty text",
      "non-empty" in _dw._worker_add_note(909, "   "))
check("live-steer: set_plan refuses unknown pid", "no worker" in _dw._worker_set_plan(404, "x"))
# planview: the .brief.planview mirror is registered + drives set_plan's read/edit affordance
check("live-steer: .brief.planview registered for cleanup",
      ".brief.planview" in lc._WORKER_SIDECAR_SUFFIXES)
check("live-steer: set_plan with no plan + no planview -> nothing to edit",
      "no pinned plan" in _dw._worker_set_plan(909, ""))
Path(str(_lsbrief) + ".planview").write_text("GOAL: live\n- [x] a\n- [ ] b\n")
_rv = _dw._worker_set_plan(909, "")   # read affordance: hand back current plan to edit
check("live-steer: set_plan with no plan returns live plan for editing",
      "current plan" in _rv and "- [x] a" in _rv)
# a finished worker (result present) can't be steered
_lsres.write_text("===RESULT===\ndone\n===RESULT===")
check("live-steer: set_plan refuses finished worker",
      "already finished" in _dw._worker_set_plan(909, "y"))
_dw._worker_rows = _saved_rows
_dw._H["workers"] = {}
_dwcfg.leash = "rwe"
# Phase 1: dead-worker RESUME (transcript checkpoint + action='resume').
# (a) checkpoint sidecar registered for cleanup or it leaks past every reap.
check("resume: .brief.checkpoint registered for cleanup",
      ".brief.checkpoint" in lc._WORKER_SIDECAR_SUFFIXES)
# (b) compose_brief carries checkpoint:1 + a RESUME block, and it PARSES BACK (a resumed
#     worker's run_agent_brief reads the checkpoint path + checkpoint flag off the grant/
#     marker) - covering parse-back, not just compose.
_rbrief = _dw._compose_brief("fix it", "", "/tmp", 10, checkpoint=True,
                             resume="/tmp/x.brief.checkpoint")
check("resume: checkpoint flag flows into GRANT block", "checkpoint: 1" in _rbrief)
check("resume: RESUME block carries the checkpoint path",
      lc._extract_marked(_rbrief, lc.RESUME_MARK) == "/tmp/x.brief.checkpoint")
_rgrant = lc._parse_grant(lc._extract_marked(_rbrief, lc.GRANT_MARK))
check("resume: worker parses checkpoint flag back from grant",
      _rgrant.get("checkpoint") == "1")
# no checkpoint/resume -> neither appears (everyday worker unchanged)
_nb = _dw._compose_brief("t", "", "/tmp", 10)
check("resume: no checkpoint/resume -> no RESUME block + no checkpoint line",
      lc.RESUME_MARK not in _nb and "checkpoint:" not in _nb)
# (c) _worker_resume guards: unknown pid, missing checkpoint, still-running, finished.
_rstmp = Path(tempfile.mkdtemp(prefix="lc_resume_"))
_rsres = _rstmp / "wr.result"
_rsbrief = _rstmp / "wr.brief"
_rsbrief.write_text(f"{lc.BRIEF_MARK}\ndo\n{lc.BRIEF_MARK}\n"
                    f"{lc.GRANT_MARK}\nleash: r\ncwd: /tmp\nmax_iterations: 10\n{lc.GRANT_MARK}\n")
check("resume: refuses unknown pid", "no worker" in _dw._worker_resume(404, "go", "/tmp"))
_saved_rows2 = _dw._worker_rows
_dw._worker_rows = lambda: {}          # not running (gone)
_dw._H["workers"] = {707: {"result": str(_rsres), "task": "t", "model": "m", "leash": "r",
                           "brief": str(_rsbrief), "checkpoint": True,
                           "cmd": "x", "launch_cwd": "/tmp"}}
check("resume: refuses when no checkpoint file exists",
      "no transcript checkpoint" in _dw._worker_resume(707, "go", "/tmp"))
_dw._worker_rows = lambda: {707: {"pid": 707, "state": "running", "runtime": "1s"}}
Path(str(_rsbrief) + ".checkpoint").write_text('{"messages": []}')
check("resume: refuses a still-running worker",
      "still running" in _dw._worker_resume(707, "go", "/tmp"))
_dw._worker_rows = lambda: {}
_rsres.write_text(f"{lc.RESULT_MARK}\ndone\n{lc.RESULT_MARK}")
check("resume: refuses a worker that already finished with a result",
      "already finished" in _dw._worker_resume(707, "go", "/tmp"))
# resume rebuilds a brief reusing the dead worker's GRANT + a RESUME marker; with an
# `iterations` grant it BUMPS max_iterations (a worker that died on its cap needs more, or
# it dies on the same cap again - found via a live dry-run). Stub bg_launch to capture the
# brief instead of launching a real process.
_rsres.unlink()                        # back to incomplete (no result) so resume proceeds
_dw._worker_rows = lambda: {}
_saved_launch = _dw._H["bg_launch"]
_captured = {}
def _fake_launch(cmd, **kw):
    _captured["cmd"] = cmd
    _captured["brief"] = kw.get("heartbeat_file", "").replace(".progress", "")
    return {"pid": 4242, "log": "/tmp/x.log"}
_dw._H["bg_launch"] = _fake_launch
_dw._H["bg_list"] = lambda kind=None: []
import shlex as _shlex_rs
_dw._H["workers"] = {707: {"result": str(_rsres), "task": "t", "model": "m", "leash": "r",
                           "brief": str(_rsbrief), "checkpoint": True,
                           "cmd": f"lc --agent-run --brief-file {_shlex_rs.quote(str(_rsbrief))} "
                                  f"--result-file {_shlex_rs.quote(str(_rsres))} --cwd /tmp",
                           "launch_cwd": "/tmp"}}
_rout = _dw._worker_resume(707, "keep going", "/tmp", iterations=25)
check("resume: relaunches as a NEW pid", "NEW pid 4242" in _rout)
_newbrief = Path(_captured["brief"]).read_text() if _captured.get("brief") else ""
check("resume: rebuilt brief carries a RESUME marker at the checkpoint",
      lc._extract_marked(_newbrief, lc.RESUME_MARK) == str(_rsbrief) + ".checkpoint")
check("resume: the steer becomes the resumed brief's task",
      lc._extract_marked(_newbrief, lc.BRIEF_MARK) == "keep going")
check("resume: iterations grant BUMPS max_iterations (not the stale 10)",
      "max_iterations: 25" in _newbrief and "max_iterations: 10" not in _newbrief)
check("resume: resumed worker is itself resumable (checkpoint:1 kept)",
      "checkpoint: 1" in lc._extract_marked(_newbrief, lc.GRANT_MARK))
check("resume: new worker's lineage is tracked",
      _dw._H["workers"][4242].get("resumed_from") == 707)
check("resume: bad iterations -> error",
      "bad iterations" in _dw._worker_resume(707, "go", "/tmp", iterations="abc"))
_dw._H["bg_launch"] = _saved_launch
_dw._worker_rows = _saved_rows2
_dw._H["workers"] = {}
# Phase 2: shared swarm BOARD (claims-with-TTL, first-claim-wins). Core primitive.
_bsess = 987654                       # a throwaway board session id
lc._board_teardown(_bsess)            # ensure clean slate
_bnow = 1_000_000.0
_ok, _holder = lc._board_claim(_bsess, "src/a.py", "worker:1", now=_bnow)
check("board: first claim on a free path succeeds", _ok and _holder == "worker:1")
_ok2, _holder2 = lc._board_claim(_bsess, "src/a.py", "worker:2", now=_bnow + 1)
check("board: second worker's claim on a held path is REFUSED",
      (not _ok2) and _holder2 == "worker:1")
_okself, _ = lc._board_claim(_bsess, "src/a.py", "worker:1", now=_bnow + 2)
check("board: re-claim by the SAME owner is allowed (refresh)", _okself)
_live = lc._board_read_claims(_bsess, now=_bnow + 3)
check("board: read_claims reduces to the live holder", _live.get("src/a.py", {}).get("owner") == "worker:1")
# release frees the path for a peer
lc._board_release(_bsess, "src/a.py", "worker:1", now=_bnow + 4)
check("board: released path drops from live claims",
      "src/a.py" not in lc._board_read_claims(_bsess, now=_bnow + 5))
_okr, _ = lc._board_claim(_bsess, "src/a.py", "worker:2", now=_bnow + 6)
check("board: a peer can claim a released path", _okr)
# TTL expiry: a stale claim frees the path (dead-worker liveness signal)
lc._board_claim(_bsess, "src/b.py", "worker:9", ttl=100, now=_bnow)
check("board: claim is held before its TTL lapses",
      "src/b.py" in lc._board_read_claims(_bsess, now=_bnow + 50))
check("board: claim EXPIRES after its TTL (frees a dead worker's path)",
      "src/b.py" not in lc._board_read_claims(_bsess, now=_bnow + 200))
_okexp, _ = lc._board_claim(_bsess, "src/b.py", "worker:2", now=_bnow + 200)
check("board: an expired path can be re-claimed by another worker", _okexp)
# corrupt line is skipped, not fatal
with lc._board_claims_path(_bsess).open("a") as _bf:
    _bf.write("{not json\n")
check("board: a corrupt claims line is skipped (reader never crashes)",
      isinstance(lc._board_read_claims(_bsess, now=_bnow + 210), dict))
# TRUE-CONCURRENCY race: N processes all claim ONE path at the same instant (barrier).
# The bug this guards (found in a Phase 2b dry-run): _board_claim used to return True to
# EVERY racer (no read-back), so all N believed they won while the reducer named just one.
# The append-only log + first-live-claim-IN-FILE-ORDER reduction (not ts - clock skew would
# let a late-appended earlier ts steal a held path) must resolve to EXACTLY ONE winner, and
# every loser must be told that same winner.
import multiprocessing as _mp_b
def _race_claim(_wid, _sid, _bar, _q):
    import sys as _s; _s.path.insert(0, str(Path(__file__).resolve().parent.parent))
    import lean_coder as _lc2
    _bar.wait()
    _q.put((_wid, _lc2._board_claim(_sid, "hot.py", f"w{_wid}")))
_race_sess = 424243
lc._board_teardown(_race_sess)
_N = 6
_bar = _mp_b.Barrier(_N); _rq = _mp_b.Queue()
_ps = [_mp_b.Process(target=_race_claim, args=(_i, _race_sess, _bar, _rq)) for _i in range(_N)]
for _p in _ps: _p.start()
for _p in _ps: _p.join()
_rr = {}
while not _rq.empty():
    _wid, _res = _rq.get(); _rr[_wid] = _res
_winners = [w for w, (ok, _h) in _rr.items() if ok]
_live_owner = lc._board_read_claims(_race_sess).get("hot.py", {}).get("owner")
check("board: a true N-way race resolves to EXACTLY ONE winner",
      len(_winners) == 1, f"winners={_winners}")
check("board: the race winner matches the reduced live owner",
      _winners and f"w{_winners[0]}" == _live_owner, f"winner=w{_winners} live={_live_owner}")
check("board: every race loser is told the actual winner",
      all(_h == _live_owner for _w, (ok, _h) in _rr.items() if not ok),
      f"loser holders={[(_w, _h) for _w,(ok,_h) in _rr.items() if not ok]}")
lc._board_teardown(_race_sess)
# teardown removes the whole board dir (zero trace)
check("board: dir exists before teardown", lc._board_dir(_bsess).is_dir())
lc._board_teardown(_bsess)
check("board: teardown leaves zero trace", not lc._board_dir(_bsess).exists())
# a crashed driver's claims board goes with its run dir (pid-liveness, never age); a live
# driver's board - however old its claims - is kept
_bdead = subprocess.Popen(["true"]); _bdead.wait()
lc._board_claim(_bdead.pid, "x", "w:1"); lc._board_claim(lc.os.getpid(), "y", "w:1")
import os as _os_b, time as _time_b
_bmine = lc._board_claims_path(lc.os.getpid())
_os_b.utime(_bmine, (_time_b.time() - 7200, _time_b.time() - 7200))  # 2h old, still live
lc._run_reap()
check("board: a dead driver's claims board is reaped with its run dir",
      not lc._board_dir(_bdead.pid).exists())
check("board: a live driver's (old) claims board is kept", lc._board_dir(lc.os.getpid()).is_dir())
lc._board_teardown(lc.os.getpid())
# the board session id flows into + parses back out of the GRANT
_bbrief = _dw._compose_brief("t", "", "/tmp", 10, board=4242)
check("board: session id flows into GRANT block", "board: 4242" in _bbrief)
check("board: worker parses board session back from grant",
      lc._parse_grant(lc._extract_marked(_bbrief, lc.GRANT_MARK)).get("board") == "4242")
# the NAMED task-board (Phase 2b) flows into + parses back out of the GRANT, distinct
# from the int claim-session above
_tbbrief = _dw._compose_brief("t", "", "/tmp", 10, taskboard="myboard")
check("board: task-board name flows into GRANT block", "taskboard: myboard" in _tbbrief)
check("board: worker parses task-board name back from grant",
      lc._parse_grant(lc._extract_marked(_tbbrief, lc.GRANT_MARK)).get("taskboard") == "myboard")
check("board: no taskboard arg -> no taskboard grant line",
      "taskboard:" not in _dw._compose_brief("t", "", "/tmp", 10))

# --- Task DAG board (Phase 2b) core primitives -------------------------------------
# Session-shaped named JSON board the DRIVER schedules over (push model). Point
# TASKBOARDS_DIR under the already-redirected temp CONFIG_DIR for the duration.
_orig_tbdir = lc.TASKBOARDS_DIR
lc.TASKBOARDS_DIR = lc.CONFIG_DIR / "workers" / "taskboards"
# create: no-clobber + name safety (reuses session-name sanitize: traversal neutralized,
# empty rejected).
_tb, _tberr = lc._taskboard_create("build-app", title="ship it", owner="drv1")
check("taskboard: create returns an empty board", (not _tberr) and _tb and _tb["tasks"] == [])
check("taskboard: create never clobbers an existing board",
      lc._taskboard_create("build-app")[1].endswith("already exists"))
check("taskboard: an all-unsafe name is rejected",
      lc._taskboard_create("///")[1] == "invalid board name")
check("taskboard: path traversal in a name is neutralized, not honored",
      lc._taskboard_path("../../etc/x").name == "etc-x.json")
# add tasks + dep validation + monotonic ids
_tA, _ = lc._taskboard_add(_tb, "build-db")
_tD, _ = lc._taskboard_add(_tb, "confirm-db-type")
_tB, _ = lc._taskboard_add(_tb, "test-db", deps=[_tA["id"]])
_tC, _ = lc._taskboard_add(_tb, "build-ui", deps=[_tD["id"]])
check("taskboard: task ids are monotonic (t1..tN)",
      [_tA["id"], _tD["id"], _tB["id"], _tC["id"]] == ["t1", "t2", "t3", "t4"])
check("taskboard: add refuses an unknown dep id",
      "unknown dep" in lc._taskboard_add(_tb, "bad", deps=["t99"])[1])
_tE, _ = lc._taskboard_add(_tb, "E", deps=[_tB["id"]])
# cycle guard: E already deps B; a hypothetical B->E edge would cycle
check("taskboard: cycle guard detects a would-be cycle",
      lc._tb_has_cycle(_tb, new_from=_tB["id"], new_to=_tE["id"]) is True)
check("taskboard: a healthy dep graph has no cycle", lc._tb_has_cycle(_tb) is False)
# ready/blocked: open tasks with all deps done are ready; the rest blocked
_tready, _tblocked = lc._taskboard_ready(_tb)
check("taskboard: ready = open tasks whose deps are all done",
      {t["name"] for t in _tready} == {"build-db", "confirm-db-type"})
check("taskboard: tasks with unmet deps are blocked (not ready)",
      {t["name"] for t in _tblocked} >= {"test-db", "build-ui", "E"})
# completing a dep promotes its dependent to ready
lc._taskboard_set_status(_tb, _tA["id"], "done", result_ref="/tmp/A.result")
check("taskboard: a dependent becomes ready once its dep is done",
      any(t["name"] == "test-db" for t in lc._taskboard_ready(_tb)[0]))
check("taskboard: set_status rejects an invalid status",
      "invalid status" in lc._taskboard_set_status(_tb, _tA["id"], "nope")[1])
check("taskboard: set_status rejects an unknown task id",
      "unknown task" in lc._taskboard_set_status(_tb, "t999", "done")[1])
# save/load roundtrip + cached counts recomputed on save
lc._taskboard_save("build-app", _tb)
_trl = lc._taskboard_load("build-app")
check("taskboard: save/load roundtrips the tasks", len(_trl["tasks"]) == 5)
check("taskboard: meta.counts is recomputed on save",
      _trl["meta"]["counts"]["done"] == 1 and _trl["meta"]["counts"]["open"] == 4)
# reconciler v1: done result_refs in dependency (topological) order
lc._taskboard_set_status(_trl, _tD["id"], "done", result_ref="/tmp/D.result")
lc._taskboard_set_status(_trl, _tC["id"], "done", result_ref="/tmp/C.result")
_trec = [t["id"] for t, _rr in lc._taskboard_reconcile(_trl)]
check("taskboard: reconcile orders a dep before its dependent (D before C)",
      _trec.index("t2") < _trec.index("t4"))
check("taskboard: reconcile only includes done tasks that have a result_ref",
      set(_trec) == {"t1", "t2", "t4"})
# defensive hand-off load: stale assigned/blocked reset to open; terminal facts kept
lc._taskboard_set_status(_trl, _tE["id"], "assigned", assignee=1234)
lc._taskboard_save("build-app", _trl)
_tdb, _ = lc._taskboard_load_defensive("build-app")
check("taskboard: defensive load resets a stale assignee to open/unassigned",
      lc._tb_task(_tdb, _tE["id"])["status"] == "open"
      and lc._tb_task(_tdb, _tE["id"])["assignee"] is None)
check("taskboard: defensive load preserves terminal (done) tasks",
      lc._tb_task(_tdb, _tA["id"])["status"] == "done")
check("taskboard: list shows the saved board", lc._taskboards_list()[0][0] == "build-app")
# corrupt file degrades to an empty board, never raises
lc._taskboard_path("build-app").write_text("{not json")
check("taskboard: a corrupt board file degrades to empty (never crashes)",
      lc._taskboard_load("build-app")["tasks"] == [])

# --- the `board` lean-tool (thin router over the core primitives) ------------------
import re as _re_bt
def _plain(s): return _re_bt.sub(r"\x1b\[[0-9;]*m", "", s or "")
_bt = lc._load_lean_tool(Path(__file__).resolve().parent.parent / "lean-tools" / "board.py")
check("board tool: is safe (workers may coordinate)", _bt.TOOL.get("safe") is True)
check("board tool: not initialised before setup -> guarded",
      "not initialised" in _bt.run({"action": "list", "board": "x"}, "/tmp"))
# DRIVER (worker_depth 0) can do everything
_btdcfg = lc.Config(); _btdcfg.worker_depth = 0
_bt.setup(lc.__dict__, _btdcfg)
check("board tool: requires a board name", "needs a 'board' name" in _bt.run({"action": "list"}, "/tmp"))
check("board tool: create makes a board",
      "created board" in _bt.run({"action": "create", "board": "swarm", "title": "t"}, "/tmp"))
check("board tool: acting on a missing board errors",
      "no board named" in _bt.run({"action": "list", "board": "ghost"}, "/tmp"))
_bt.run({"action": "add", "board": "swarm", "task": "db"}, "/tmp")            # t1
_bt.run({"action": "add", "board": "swarm", "task": "ui", "deps": ["t1"]}, "/tmp")  # t2 blocked
check("board tool: add reports READY for a dep-free task",
      "READY" in _plain(_bt.run({"action": "add", "board": "swarm", "task": "docs"}, "/tmp")))
check("board tool: add reports blocked for an unmet-dep task",
      "blocked" in _plain(_bt.run({"action": "add", "board": "swarm", "task": "test",
                                   "deps": ["t2"]}, "/tmp")))
check("board tool: add refuses an unknown dep",
      "unknown dep" in _bt.run({"action": "add", "board": "swarm", "task": "z",
                                "deps": ["t99"]}, "/tmp"))
check("board tool: list ready shows only ready tasks",
      "t2" not in _plain(_bt.run({"action": "list", "board": "swarm", "status": "ready"}, "/tmp")))
check("board tool: assign refuses a task whose deps aren't done",
      "is not ready" in _bt.run({"action": "assign", "board": "swarm", "task": "t2",
                                 "worker": "100"}, "/tmp"))
check("board tool: assign puts a worker on a ready task",
      "assigned t1" in _bt.run({"action": "assign", "board": "swarm", "task": "t1",
                                "worker": "100"}, "/tmp"))
check("board tool: done reports what it unblocked",
      "ready now" in _bt.run({"action": "done", "board": "swarm", "task": "t1",
                              "result_ref": "/tmp/t1.r"}, "/tmp"))
check("board tool: list query= matches by text",
      "t2" in _plain(_bt.run({"action": "list", "board": "swarm", "query": "ui"}, "/tmp")))
check("board tool: list query= with no match says so",
      "no task matches" in _bt.run({"action": "list", "board": "swarm", "query": "zzz"}, "/tmp"))
# WORKER (worker_depth 1): coordination is refused, self-report + read allowed
_btwcfg = lc.Config(); _btwcfg.worker_depth = 1
_bt.setup(lc.__dict__, _btwcfg)
check("board tool: a worker cannot create",
      "driver-only" in _bt.run({"action": "create", "board": "sneaky"}, "/tmp"))
check("board tool: a worker cannot add",
      "driver-only" in _bt.run({"action": "add", "board": "swarm", "task": "x"}, "/tmp"))
check("board tool: a worker cannot assign",
      "driver-only" in _bt.run({"action": "assign", "board": "swarm", "task": "t2",
                                "worker": "9"}, "/tmp"))
check("board tool: a worker CAN list (read is open)",
      "board 'swarm'" in _plain(_bt.run({"action": "list", "board": "swarm"}, "/tmp")))
check("board tool: a worker CAN mark its own task done",
      "DONE" in _bt.run({"action": "done", "board": "swarm", "task": "t2",
                         "result_ref": "/tmp/t2.r"}, "/tmp"))
# reconcile: a read action open to anyone (here a worker), collects done result_refs in
# dependency order - t1 (dep) before t2 (dependent). Phase 2c driver-facing surface.
_btrec = _plain(_bt.run({"action": "reconcile", "board": "swarm"}, "/tmp"))
check("board tool: a worker CAN reconcile (read is open)", "reconcile" in _btrec)
check("board tool: reconcile lists both result_refs", "/tmp/t1.r" in _btrec and "/tmp/t2.r" in _btrec)
check("board tool: reconcile orders a dep before its dependent (t1 before t2)",
      _btrec.index("/tmp/t1.r") < _btrec.index("/tmp/t2.r"))
# 1a: assign auto-pings a LIVE worker with the task detail inline (via the core
# worker_inject bridge) so it acts this turn instead of polling the board. A non-pid
# label (a peer role, an unspawned worker) just falls back to no push - board still works.
_pushed = []
lc.register_worker_inject(lambda w, t: (_pushed.append((w, t)) or True) if str(w).strip().isdigit() else False)
_bt.setup(lc.__dict__, _btdcfg)   # re-setup as DRIVER so worker_inject hook is captured
_bt.run({"action": "add", "board": "swarm", "task": "extra"}, "/tmp")   # -> t5, ready
_pmsg = _bt.run({"action": "assign", "board": "swarm", "task": "t5",
                 "worker": "777", "note": "hurry"}, "/tmp")
check("board tool: assign to a live pid pings the worker (1a)",
      _pushed and _pushed[-1][0] == "777" and "assigned t5" in _pushed[-1][1]
      and "worker pinged" in _pmsg)
_before = len(_pushed)
_bt.run({"action": "add", "board": "swarm", "task": "extra2"}, "/tmp")
_bt.run({"action": "assign", "board": "swarm", "task": "t6", "worker": "peer-dev"}, "/tmp")
check("board tool: assign to a non-pid label does NOT push (falls back)",
      len(_pushed) == _before)
lc.register_worker_inject(lambda w, t: False)   # reset bridge so later tests are unaffected
# Participant registry: a peer is identified by its session-file NAME (durable), its live
# ADDRESS (pid) resolved on demand from the session lock - no pid/host stored on the board.
# _participant_resolve gives live-here / live-elsewhere / dormant; assign branches on it.
_pb = {"meta": {}, "tasks": []}
_prec, _perr = lc._taskboard_participant_upsert(_pb, "plugin-dev", role="plugin-dev")
check("participant upsert: record is identity only (no liveness: name/role/note)",
      _perr == "" and set(_prec) == {"name", "role", "note"})
check("participant lookup: by name and by role",
      lc._taskboard_participant(_pb, "plugin-dev") is _prec
      and lc._taskboard_participant(_pb, "plugin-dev")["role"] == "plugin-dev")
check("participant resolve: unknown name, no session file -> missing (deleted?)",
      lc._participant_resolve("no-such-peer-xyz")["state"] == "missing")
# resolve states via a synthetic session file + lock. dormant requires the FILE to exist
# (a deleted session resolves to 'missing', not 'dormant' - the roster entry is kept but the
# state is legible so the model isn't confused by a peer that silently vanished).
lc.SESSIONS_DIR.mkdir(parents=True, exist_ok=True)
_psess = lc._session_path("smoketest_peer_xyz")
_psess.write_text(lc.json.dumps({"meta": {}, "messages": []}))
_plk = lc._lock_path("smoketest_peer_xyz")
check("participant resolve: file exists, no lock -> dormant (spawnable)",
      lc._participant_resolve("smoketest_peer_xyz")["state"] == "dormant")
_plk.write_text(lc.json.dumps({"pid": 1, "host": lc._HOSTNAME, "ts": lc.time.time()}))
check("participant resolve: alive pid same host -> live-here",
      lc._participant_resolve("smoketest_peer_xyz")["state"] == "live-here")
_plk.write_text(lc.json.dumps({"pid": 999999, "host": lc._HOSTNAME, "ts": lc.time.time()}))
check("participant resolve: dead pid same host, file exists -> dormant",
      lc._participant_resolve("smoketest_peer_xyz")["state"] == "dormant")
_plk.write_text(lc.json.dumps({"pid": 50, "host": "otherbox", "ts": lc.time.time()}))
check("participant resolve: fresh lock other host -> live-elsewhere",
      lc._participant_resolve("smoketest_peer_xyz")["state"] == "live-elsewhere")
_plk.write_text(lc.json.dumps({"pid": 50, "host": "otherbox", "ts": lc.time.time() - 99999}))
check("participant resolve: stale lock other host, file exists -> dormant",
      lc._participant_resolve("smoketest_peer_xyz")["state"] == "dormant")
_plk.unlink()
_psess.unlink()
check("participant resolve: file deleted -> missing (not dormant)",
      lc._participant_resolve("smoketest_peer_xyz")["state"] == "missing")
# assign to a registered participant branches on resolver state: dormant -> spawn_peer,
# live-here -> push. Stub both bridges. Give peerA a session file so it resolves dormant.
_spawned = []
lc.register_peer_spawn(lambda s, t: (_spawned.append((s, t)) or f"dispatched worker pid 9 from_session={s}"))
lc.register_worker_inject(lambda w, t: (_pushed.append((w, t)) or True) if str(w).strip().isdigit() else False)
_bt.setup(lc.__dict__, _btdcfg)
_peerA_sess = lc._session_path("peerA")
_peerA_sess.write_text(lc.json.dumps({"meta": {}, "messages": []}))
_bt.run({"action": "add", "board": "swarm", "task": "peer job"}, "/tmp")   # -> t7
_bt.run({"action": "participant", "board": "swarm", "worker": "peerA", "role": "peerA"}, "/tmp")
_pasg = _bt.run({"action": "assign", "board": "swarm", "task": "t7", "worker": "peerA"}, "/tmp")
check("board tool: assign to a DORMANT participant spawns it from its session",
      _spawned and _spawned[-1][0] == "peerA" and "dormant" in _pasg and "spawned" in _pasg)
check("board tool: participant list shows the registered peer + resolved state",
      "peerA" in _bt.run({"action": "participant", "board": "swarm"}, "/tmp"))
# DUPE GUARD (the courseloop race): a peer already being driven by a LIVE worker
# (from_session=) still resolves 'dormant' (a worker holds no session lock), so a
# second assign must NOT spawn a duplicate - it pings the existing worker instead.
lc.SESSIONS_DIR.mkdir(parents=True, exist_ok=True)
_peerA_sess.write_text(lc.json.dumps({"meta": {}, "messages": []}))   # dormant file again
_dg_reg_save = lc._bg_load()                       # snapshot; restore after (record below leaks)
lc._bg_register(lc.os.getppid(), "lean_coder --agent-run peerA", _exlog + ".peerAwkr",
                kind="worker", meta={"from_session": "peerA"})
_spawn_before = len(_spawned)
_dupadd = _bt.run({"action": "add", "board": "swarm", "task": "peer job dup"}, "/tmp")
_duptid = _dupadd.split()[1]   # "added tN 'name'..." -> tN
_dupasg = _bt.run({"action": "assign", "board": "swarm",
                   "task": _duptid, "worker": "peerA"}, "/tmp")
check("board tool: assign to a peer already driven by a live worker does NOT re-spawn",
      len(_spawned) == _spawn_before and "re-spawned" in _dupasg
      and str(lc.os.getppid()) in _dupasg)
_bg_restore(_dg_reg_save)                           # restore registry (drop the test worker rec)
# assign to a MISSING peer (session deleted): kept on roster, but reports not-spawnable.
_peerA_sess.unlink()
_before_spawn = len(_spawned)
_pmiss = _bt.run({"action": "add", "board": "swarm", "task": "peer job 2"}, "/tmp")   # -> t8
_bt.run({"action": "add", "board": "swarm", "task": "z2"}, "/tmp")
_pmiss = _bt.run({"action": "assign", "board": "swarm", "task": "t8", "worker": "peerA"}, "/tmp")
check("board tool: assign to a MISSING peer does not spawn + says deleted",
      len(_spawned) == _before_spawn and "no session file" in _pmiss)
_btwcfg2 = lc.Config(); _btwcfg2.worker_depth = 1
_bt.setup(lc.__dict__, _btwcfg2)
check("board tool: a worker cannot register a participant (driver-only write)",
      "driver-only" in _bt.run({"action": "participant", "board": "swarm", "worker": "sneak"}, "/tmp"))
_bt.setup(lc.__dict__, _btdcfg)   # restore driver setup for any later use
_btwcfg2 = lc.Config(); _btwcfg2.worker_depth = 1
_bt.setup(lc.__dict__, _btwcfg2)
check("board tool: a worker cannot register a participant (driver-only write)",
      "driver-only" in _bt.run({"action": "participant", "board": "swarm", "worker": "sneak"}, "/tmp"))
_bt.setup(lc.__dict__, _btdcfg)   # restore driver setup for any later use
lc.register_peer_spawn(lambda s, t: "")
lc.register_worker_inject(lambda w, t: False)
lc.TASKBOARDS_DIR = _orig_tbdir
lc.TASKBOARDS_DIR = _orig_tbdir
...
_bg_reset()                                        # tidy the registry for later checks
...
# Per-process run dirs: reap / adopt / legacy cleanup. Fake a DEAD owner's dir holding a
# LIVE detached task (a real sleep), then check who may kill / adopt it.
def _fake_dead_dir(role=None, watchdog=None):
    _dp = subprocess.Popen(["true"]); _dp.wait()                 # a certainly-dead pid
    _dd = lc._run_pid_dir(_dp.pid); _dd.mkdir(parents=True)
    _sp = subprocess.Popen(["sleep", "30"], start_new_session=True)
    _rec = {"pid": _sp.pid, "cmd": "sleep 30", "log": str(_dd / "t.log"),
            "owner": _dp.pid, "host": lc._HOSTNAME, "kind": "task"}
    if role:
        _rec.update(role=role, watchdog=watchdog)
    lc._bg_write_rec(_rec)
    return _dd, _sp
_deadpid = subprocess.Popen(["true"]); _deadpid.wait()
# (a) REPL reaper: a crashed REPL's dir -> its task is killed and the dir removed
_d1, _s1 = _fake_dead_dir()
check("run reap: dead REPL dir's live task is killed", lc._run_reap() >= 1)
_s1.wait(timeout=5)
check("run reap: dead REPL dir removed", not _d1.exists())
# (b) REPL reaper must NOT touch a dropped EXECUTOR's survivors while its watchdog lives
_d2, _s2 = _fake_dead_dir(role="executor", watchdog=1)          # pid 1: always alive
lc._run_reap()
check("run reap: executor survivors kept while their watchdog lives",
      _d2.exists() and lc._proc_alive(_s2.pid))
# (c) ...but once the watchdog is gone too, they are reaped
lc._bg_write_rec(dict(lc._bg_load(_d2)[0], watchdog=_deadpid.pid))
lc._run_reap()
_s2.wait(timeout=5)
check("run reap: executor survivors reaped once the watchdog is dead", not _d2.exists())
# (d) executor adopt: a dropped executor's survivors are re-parented, not killed
_d3, _s3 = _fake_dead_dir(role="executor", watchdog=1)
lc._bg_adopt()
_a3 = lc._bg_load(_d3)
check("run adopt: executor survivor re-parented to me (not killed)",
      _d3.exists() and lc._proc_alive(_s3.pid) and _a3 and _a3[0]["owner"] == lc.os.getpid()
      and lc._run_dir_owner(_d3) == lc.os.getpid())
check("run adopt: adopted dir counts as mine for teardown", _d3 in lc._run_owned_dirs())
# (e) executor adopt must NOT adopt a crashed REPL's dir on the same box - it reaps it
_d4, _s4 = _fake_dead_dir()
lc._bg_adopt()
_s4.wait(timeout=5)
check("run adopt: a crashed REPL dir is reaped, never adopted", not _d4.exists())
# (f) teardown of everything I own (own + adopted) kills the adopted survivor
lc._bg_kill_session()
_s3.wait(timeout=5)
check("run teardown: clean exit kills + removes adopted dirs too", not _d3.exists())
# (g) wipe watchdog: executor gone for TTL -> its tasks are killed before the rmtree
_wd_pushed = Path(tempfile.mkdtemp()) / "leancoder"; _wd_pushed.mkdir()
_d5, _s5 = _fake_dead_dir(role="executor", watchdog=1)
_o5 = lc._run_dir_owner(_d5)
_old = lc.time.time() - 999
(_wd_pushed / ".lease").touch(); os.utime(_wd_pushed / ".lease", (_old, _old))
lc.run_wipe_watchdog({"dir": str(_wd_pushed), "ttl": 60, "chk": 1, "owner": _o5})
_s5.wait(timeout=5)
check("wipe watchdog: kills the dead executor's tasks + removes its run dir",
      not _d5.exists() and not _wd_pushed.exists())
# found live on a real remote: the pushed dir is REUSED across sessions, so (a) a watchdog
# must step aside once .watchdog.pid names a newer one, and (b) if another watchdog wiped
# the dir first, a watchdog whose executor is dead still reaps that executor's orphans.
_wd2 = Path(tempfile.mkdtemp()) / "leancoder"; _wd2.mkdir()
(_wd2 / ".watchdog.pid").write_text("1")                   # a NEWER watchdog owns the dir
(_wd2 / ".lease").touch(); os.utime(_wd2 / ".lease", (_old, _old))
lc.run_wipe_watchdog({"dir": str(_wd2), "ttl": 60, "chk": 1, "owner": 0})
check("wipe watchdog: a superseded watchdog steps aside (never wipes a newer session's dir)",
      _wd2.is_dir())
shutil.rmtree(_wd2.parent, ignore_errors=True)
_d6, _s6 = _fake_dead_dir(role="executor", watchdog=1)
_wd3 = Path(tempfile.mkdtemp()) / "leancoder"               # already wiped: never created
lc.run_wipe_watchdog({"dir": str(_wd3), "ttl": 60, "chk": 1, "owner": lc._run_dir_owner(_d6)})
_s6.wait(timeout=5)
check("wipe watchdog: dir already wiped -> still reaps its dead executor's orphans",
      not _d6.exists())
# (h) legacy CONFIG_DIR/bg: dead-owner files removed, a live owner's kept, dir kept
_lg = lc.CONFIG_DIR / "bg"; _lg.mkdir(parents=True, exist_ok=True)
(_lg / f"20260701-010101-{_deadpid.pid}.log").write_text("x")
(_lg / f"20260701-010101-{_deadpid.pid}.log.exit").write_text("0")
(_lg / f"20260701-010101-{lc.os.getpid()}.log").write_text("mine")
(_lg / "tasks.jsonl").write_text("")
lc._bg_legacy_cleanup()
_left = sorted(f.name for f in _lg.iterdir())
check("legacy bg cleanup: dead-owner files go, a live owner's stay",
      _left == [f"20260701-010101-{lc.os.getpid()}.log"], str(_left))
(_lg / f"20260701-010101-{lc.os.getpid()}.log").unlink()
lc._bg_legacy_cleanup()
check("legacy bg cleanup: empty legacy dir removed", not _lg.exists())
# (i) legacy CONFIG_DIR/workers: dead-driver files go (no age rule), a live driver's and
# a parked (.suspended) family stay, subdirs (taskboards/) untouched
_lw = lc.CONFIG_DIR / "workers"; (_lw / "taskboards").mkdir(parents=True, exist_ok=True)
_dst = f"20260701-010101-{_deadpid.pid}-1"
_lst = f"20260701-010101-{lc.os.getpid()}-1"
_pst = f"20260701-010101-{_deadpid.pid}-2"
for _n in (_dst + ".brief", _dst + ".result", _dst + ".brief.progress.silent",
           _lst + ".brief", _pst + ".brief.checkpoint", _pst + ".brief.suspended"):
    (_lw / _n).write_text("x")
lc._worker_dir_sweep()
_wleft = sorted(f.name for f in _lw.iterdir())
check("legacy workers cleanup: dead driver's family removed (incl .progress.silent)",
      not any(n.startswith(_dst) for n in _wleft), str(_wleft))
check("legacy workers cleanup: live driver's + parked (.suspended) files kept, taskboards/ kept",
      _lst + ".brief" in _wleft and _pst + ".brief.checkpoint" in _wleft and "taskboards" in _wleft)
_bg_reset()                                            # cleanup
...
_lmsg = lc._bg_finished_msg([{"cmd": "x", "code": "0", "tail": "t", "log": "/r/x.log", "lines": 152}])
check("bg finished notice: a long log names the path + how to page it",
      "last 20 of 152 lines; full log: read_file path=/r/x.log" in _lmsg, _lmsg)
check("bg finished notice: a short log adds no pointer",
      "full log" not in lc._bg_finished_msg([{"cmd": "x", "code": "0", "tail": "t",
                                               "log": "/r/x.log", "lines": 5}]))
...
lc._bg_kill_session(); _bg_reset()                 # cleanup
...
# atomic write (temp + fsync + rename): a crash mid-write must not truncate config.toml
# and break the next launch. Assert no .tmp sidecar is left behind after a save (the
# rename either fully lands or leaves the prior file intact - never a torn config).
check("save_config: no leftover .tmp sidecar after save (atomic write)",
      not any(p.name.startswith(_tcf.name + ".tmp") for p in _tcf.parent.iterdir()))
...
lc.save_config(lc.Config(model="m", window_messages=50, compact_soft=0.4, compact_at=0.6),
               quiet=True)
_rtw = _tt_bg.loads(_tcf.read_text())
check("save_config: window_messages round-trip", _rtw.get("window_messages") == 50)
check("save_config: compact_soft/at round-trip",
      _rtw.get("compact_soft") == 0.4 and _rtw.get("compact_at") == 0.6)
...
# ==========================================================================
# SECTION: Config drift guard + queue-drain + host normalization
# ==========================================================================
...
_nondefault = {
    ...
    "autosave": False, "auto_update": True, "update_track": "beta", "command_timeout": 99, "tool_timeout": 45,
    "last_seen_version": "9.9.9",
    "bg_max_concurrent": 2, "worker_max_concurrent": 4, "worker_idle_timeout": 900,
    "worker_max_iterations": 15, "worker_max_depth": 4, "worker_max_children": 2,
    "worker_checkpoint": True,
    "editor": "vim", "approval": "auto", "confirm_reads": True,
    "auto_reconnect": True, "statusline": False, "statusline_every": 3, "statusline_iter": 20,
    "output_wrap": "auto",
    "window_messages": 12, "window_tokens": 4000,
    "auto_compact": False,
    "compact_soft": 0.42, "compact_at": 0.61, "compact_soft_ratio": 0.75, "compact_emergency": 0.99,
    "compact_min_interval": 33.0, "autostart_after_compact": False,
    "compact_keep": 5, "keep_cap": 18000,
    ...
    "capture_training": True, "capture_training_dir": "/tmp/lc-train",
    "scrub_invisible": False,
}
...
# 10c-bis. compact_at/compact_soft_ratio drive compact_soft (one lever moves the pair,
# PROPORTIONALLY: soft = at * ratio; a direct compact_soft set decouples). Session scope.
_cg = lc.Config()
lc._set_setting_field(None, _cg, "compact_at", "0.80", scope="session")
check("compact: /set compact_at slides soft (soft = at * ratio, default 0.8)",
      abs(_cg.compact_soft - 0.64) < 1e-9 and "compact_soft" in _cg.session_overrides,
      f"soft={_cg.compact_soft}, overrides={sorted(_cg.session_overrides)}")
lc._set_setting_field(None, _cg, "compact_soft_ratio", "0.5", scope="session")
check("compact: /set compact_soft_ratio re-derives soft from compact_at",
      abs(_cg.compact_soft - 0.40) < 1e-9, f"soft={_cg.compact_soft}")
lc._set_setting_field(None, _cg, "compact_soft", "0.42", scope="session")
check("compact: direct /set compact_soft decouples (compact_at unchanged)",
      abs(_cg.compact_soft - 0.42) < 1e-9 and abs(_cg.compact_at - 0.80) < 1e-9,
      f"soft={_cg.compact_soft} at={_cg.compact_at}")
# back-compat: a config with the OLD compact_hard key maps onto compact_at. Isolate on a
# scratch CONFIG_PATH so we don't disturb the shared scratch config the next tests read.
_bc_orig_cp = lc.CONFIG_PATH
_bc_cp = Path(tempfile.mkdtemp()) / "legacy.toml"
_bc_cp.write_text('compact_hard = 0.55\n')
lc.CONFIG_PATH = _bc_cp
try:
    _bcfg = lc.load_config(_Args())
    check("compact: legacy compact_hard aliases onto compact_at on load",
          abs(_bcfg.compact_at - 0.55) < 1e-9, f"compact_at={_bcfg.compact_at}")
finally:
    lc.CONFIG_PATH = _bc_orig_cp

# duplicate-key TOML recovery: a stale session can serialise a duplicate top-level key,
# which makes tomllib reject the WHOLE file. load_config must salvage the scalars (last
# value wins) instead of silently dropping the user's model pin etc to defaults.
check("_salvage_config_scalars: last value wins on a duplicate key",
      lc._salvage_config_scalars("x = 1\nx = 2\n")["x"] == 2)
check("_salvage_config_scalars: coerces bool/str/int + stops at first table",
      lc._salvage_config_scalars('a = true\nb = "hi"\nc = 3\n[t]\nd = 9\n')
      == {"a": True, "b": "hi", "c": 3})
_dup_cp = Path(tempfile.mkdtemp()) / "dup.toml"
_dup_cp.write_text('model = "qwen3-coder:30b"\ncommand_timeout = 30\n'
                   'command_timeout = 250\n[providers.ollama]\nmodel = "x"\n')
lc.CONFIG_PATH = _dup_cp
try:
    import tomllib as _tl_dup
    _raised = False
    try:
        _tl_dup.loads(_dup_cp.read_text())
    except Exception:
        _raised = True
    check("dup-config: tomllib really rejects the duplicate-key file", _raised)
    _dupcfg = lc.load_config(_Args())
    check("dup-config: load_config recovers the model pin instead of defaulting",
          _dupcfg.model == "qwen3-coder:30b")
    check("dup-config: last duplicate value wins through load_config",
          _dupcfg.command_timeout == 250)
finally:
    lc.CONFIG_PATH = _bc_orig_cp

# load-time soft derivation: a config that sets ONLY compact_at must derive compact_soft
# from the ratio (not leave it at the default, which could sit above the hard cap).
_sd_cp = Path(tempfile.mkdtemp()) / "softderive.toml"
_sd_cp.write_text('compact_at = 0.2\n')          # no compact_soft -> derive
lc.CONFIG_PATH = _sd_cp
try:
    _sdcfg = lc.load_config(_Args())
    check("compact: load derives soft from compact_at * ratio when unset (0.2*0.8=0.16)",
          abs(_sdcfg.compact_soft - 0.16) < 1e-9, f"soft={_sdcfg.compact_soft}")
    _sd_cp.write_text('compact_at = 0.2\ncompact_soft = 0.05\n')   # explicit -> decoupled
    _sd2 = lc.load_config(_Args())
    check("compact: an explicit compact_soft in config is NOT overridden by derivation",
          abs(_sd2.compact_soft - 0.05) < 1e-9, f"soft={_sd2.compact_soft}")
finally:
    lc.CONFIG_PATH = _bc_orig_cp
...
# _coalesce_queued: banked mid-turn messages fold into ONE turn; single passes bare,
# multi gets the "while you were working" frame, ^C gets the "interrupted" frame.
check("_coalesce_queued: single msg passes bare", lc._coalesce_queued(["hi"]) == "hi")
check("_coalesce_queued: multi framed + joined",
      lc._coalesce_queued(["a", "b"]).endswith("a\nb")
      and "while you were working" in lc._coalesce_queued(["a", "b"]))
check("_coalesce_queued: interrupt framed (even single)",
      "interrupted your previous turn" in lc._coalesce_queued(["stop"], interrupted=True)
      and lc._coalesce_queued(["stop"], interrupted=True).endswith("stop"))
...
try:
    ...
    check("save_config preserves non-default global thresholds",
          (_rl.compact_soft, _rl.compact_at) == (0.2, 0.25),
          (_rl.compact_soft, _rl.compact_at))
...
check("_ingest_cap emits a truncation notice the model sees", "cut at ingestion" in _capped)
check("_ingest_cap: unknown tool -> 're-call narrower' (never invents offset/grep)",
      "re-call mcp__s__t with narrower arguments" in _capped and "offset/grep" not in _capped)
# a tool that DOES page gets pointed at its real paging args
class _FakeLT:
    def schemas(self):
        return [({"function": {"name": "pager_tool", "parameters": {"properties":
                 {"url": {}, "start": {}, "find": {}}}}}, True)]
ic.lean_tools = _FakeLT()
check("_ingest_cap: a paging tool is pointed at ITS real args (start= / find=)",
      "re-call pager_tool with start= / find=" in ic._ingest_cap(_big, "pager_tool"))
del ic.lean_tools
...
# 12d-tris. Context-aware compaction keep: token-budgeted verbatim tail (replaces the old
# fixed compact_keep=3-turns). _split_into_turns / _keep_budget / _keep_tail_by_budget /
# _trim_turn_to_budget - the algorithm that keeps a multi-day turn from being useless.
kb = lc.Agent.__new__(lc.Agent)
kb.cfg = lc.Config(model="m")
kb.cfg.session_overrides = {}
kb.tool_defs = []
kb._system = lambda: "S"
# _split_into_turns: system stripped, each turn starts on a user msg
_km = [{"role": "system", "content": "s"},
       {"role": "user", "content": "u1"}, {"role": "assistant", "content": "a1"},
       {"role": "user", "content": "u2"}, {"role": "assistant", "content": "a2"}]
_kt = kb._split_into_turns(_km)
check("_split_into_turns: 2 turns, each starts user",
      len(_kt) == 2 and all(t[0]["role"] == "user" for t in _kt))
check("_split_into_turns: orphan prefix folds into first turn",
      kb._split_into_turns([{"role": "system", "content": "s"},
                            {"role": "assistant", "content": "orphan"},
                            {"role": "user", "content": "u"}])[0][0]["role"] == "assistant")
check("_split_into_turns: no user msg -> []", kb._split_into_turns([{"role": "system", "content": "s"}]) == [])
# _keep_budget: NO floor. Huge summary on a tiny window -> 0. Big window -> capped at keep_cap.
kb.cfg.num_ctx = 4000; kb.cfg.compact_at = 0.5; kb.cfg.keep_cap = 25000
check("_keep_budget: summary bigger than window -> 0 (no floor)",
      kb._keep_budget("x " * 5000) == 0)
kb.cfg.num_ctx = 1_000_000
check("_keep_budget: huge window capped at keep_cap, never frac*headroom",
      kb._keep_budget("short") <= 25000)
# _keep_tail_by_budget: a MONSTER single turn (bigger than budget) is trimmed, not kept whole
# and not dropped wholesale - the multi-day-turn case the old fixed keep couldn't handle.
kb.cfg.num_ctx = 8000; kb.cfg.compact_at = 0.5; kb.cfg.compact_keep = 3
_monster = [{"role": "system", "content": "S"},
            {"role": "user", "content": "big task"},
            {"role": "assistant", "content": "plan"},
            {"role": "tool", "tool_name": "read", "content": "X" * 60000},
            {"role": "assistant", "content": "done"}]
_mtail = kb._keep_tail_by_budget(_monster, "summary")
check("_keep_tail_by_budget: monster turn kept within budget",
      _mtail and lc.messages_tokens(_mtail, None) <= kb._keep_budget("summary"))
check("_keep_tail_by_budget: monster turn keeps user ask + omitted marker",
      any(m["role"] == "user" and "big task" in m["content"] for m in _mtail)
      and any("omitted" in str(m.get("content", "")) for m in _mtail))
# /compact <k>: k is the FINAL size (compact_at stays the trigger). The kept tail fills up
# to k (keep_cap + the compact_keep turn count lifted) and lands AT/under it.
kb.cfg.num_ctx = 1_000_000; kb.cfg.compact_at = 0.9; kb.cfg.compact_keep = 3; kb.cfg.keep_cap = 25000
_hist = [{"role": "system", "content": "S"}]
for _i in range(60):                                   # 60 turns x ~2.5k tok each
    _hist += [{"role": "user", "content": f"ask {_i}"},
              {"role": "assistant", "content": ("w%d " % _i) * 2000}]
_sumtxt = "summary " * 400
_norm = kb._keep_tail_by_budget(_hist, _sumtxt)
kb._compact_target = 100_000
_tgt_budget = kb._keep_budget(_sumtxt)
_tgt = kb._keep_tail_by_budget(_hist, _sumtxt)
kb._compact_target = None
_fixed = lc.messages_tokens([{"role": "system", "content": "S"}], [])
_sum_tok = lc.messages_tokens([{"role": "user", "content": _sumtxt}], None)
_landed = _fixed + _sum_tok + lc.messages_tokens(_tgt, None)
check("/compact <k>: normal compaction still capped (keep_cap / compact_keep=3 turns)",
      len(kb._split_into_turns([{"role": "system", "content": ""}] + _norm)) <= 3)
check("/compact <k>: target budget = target - system - summary (keep_cap lifted)",
      _tgt_budget == 100_000 - _fixed - _sum_tok, f"{_tgt_budget}")
check("/compact <k>: fills past compact_keep with whole turns",
      len(kb._split_into_turns([{"role": "system", "content": ""}] + _tgt)) > 3)
check("/compact <k>: lands at/under the target and close to it",
      95_000 <= _landed <= 100_000, f"landed ~{_landed}")
check("/compact <k>: override is one-off (cleared after)", kb._keep_budget(_sumtxt) <= 25000)
# /compact <k> with tool-heavy history: _finish_compact stubs kept tool bodies, so the tail
# must be sized in that stubbed form or it lands far under k (live: 100k asked -> 61.5k).
_th = [{"role": "system", "content": "S"}]
for _i in range(260):                                  # stubbed form alone > 100k
    _th += [{"role": "user", "content": f"ask {_i}"},
            {"role": "assistant", "content": "", "tool_calls": [{"id": f"c{_i}", "type": "function",
             "function": {"name": "read_file", "arguments": "{}"}}]},
            {"role": "tool", "tool_call_id": f"c{_i}", "tool_name": "read_file", "content": "x\n" * 6000},
            {"role": "assistant", "content": ("r%d " % _i) * 500}]
kb._compact_target = 100_000
_ttail = kb._keep_tail_by_budget(_th, _sumtxt)
kb._compact_target = None
_stubbed = lc._stub_tool_msgs(_ttail)              # what _finish_compact leaves (newest 3 whole)
_tland = _fixed + _sum_tok + lc.messages_tokens(_stubbed, None)
check("/compact <k>: tool-heavy history still lands near k AFTER tool bodies are stubbed",
      95_000 <= _tland <= 100_000, f"landed ~{_tland}")
check("/compact <k>: sizing never mutates the live history's tool bodies",
      _th[3]["content"].startswith("x\n"))
# /compact <k> handler refusals: never reaches the model (agent.compact must NOT be called)
import io as _io_ck, contextlib as _ctx_ck
class _CkAgent:
    tool_defs = []
    def __init__(self, used):
        self._u = used; self.called = False
        self.messages = [{"role": "system", "content": "S"}, {"role": "user", "content": "x" * (used * 4)}]
    def _print_ctx(self): pass
    def _ctx_used(self): return self._u
    def _system(self): return "S"
    def compact(self, target=None): self.called = True; return None
_ckcfg = lc.Config(model="m"); _ckcfg.num_ctx = 1_000_000; _ckcfg.compact_at = 0.5
def _ck(arg, used):
    _a = _CkAgent(used); _b = _io_ck.StringIO()
    with _ctx_ck.redirect_stdout(_b):
        lc.handle_compact_command(_a, _ckcfg, arg)
    return _a.called, _b.getvalue()
_c, _o = _ck("1", 300_000)
check("/compact <k>: below the minimum is refused", not _c and "below the minimum" in _o, _o)
_c, _o = _ck("400", 300_000)
check("/compact <k>: at/above current use -> nothing to do", not _c and "nothing to do" in _o, _o)
_c, _o = _ck("600", 900_000)
check("/compact <k>: at/above the compact_at trigger is refused",
      not _c and "compact_at trigger" in _o, _o)
_c, _o = _ck("abc", 300_000)
check("/compact <k>: a non-number is rejected with usage", not _c and "not a size" in _o, _o)
_c, _o = _ck("100", 300_000)
check("/compact <k>: a valid target reaches the compaction", _c)
_c, _o = _ck("100k", 300_000)
check("/compact <k>: '100k' form accepted", _c)
# /compact <k> catch: if stubbing old tool results alone lands under k, no summary is written.
_sk = lc.Agent.__new__(lc.Agent)
_sk.cfg = _ckcfg; _sk.tool_defs = []; _sk._system = lambda: "S"; _sk.last_prompt_tokens = None
_sk.messages = [{"role": "system", "content": "S"}]
for _i in range(10):
    _sk.messages += [{"role": "user", "content": f"q{_i}"},
                     {"role": "assistant", "content": "", "tool_calls": [{"id": f"k{_i}", "type": "function",
                      "function": {"name": "read_file", "arguments": "{}"}}]},
                     {"role": "tool", "tool_call_id": f"k{_i}", "tool_name": "read_file", "content": "y\n" * 20000}]
_sk.ctx = lc.ContextMeter(_sk)
_sk_called = []
_sk.compact = lambda target=None: _sk_called.append(target)
_sk._print_ctx = lambda: None
_b = _io_ck.StringIO()
with _ctx_ck.redirect_stdout(_b):
    lc.handle_compact_command(_sk, _ckcfg, "100")
_tools_left = [m for m in _sk.messages if m.get("role") == "tool"]
check("/compact <k>: fits after stubbing -> no summary call, says so",
      not _sk_called and "whole history fits after tool-result stubbing" in _b.getvalue(), _b.getvalue())
check("/compact <k>: stub-only keeps the newest TRIM_KEEP tool results whole",
      sum(not m["content"].startswith("[trimmed") for m in _tools_left) == lc.TRIM_KEEP)
check("/compact <k>: stub-only tells the model what happened",
      _sk.messages[-1]["content"].startswith("[context maintenance] old tool results stubbed"))
# ...and steps the kept-whole count down (3 -> 0) when huge RECENT results won't fit
_sk2 = lc.Agent.__new__(lc.Agent)
_sk2.cfg = _ckcfg; _sk2.tool_defs = []; _sk2._system = lambda: "S"; _sk2.last_prompt_tokens = None
_sk2.messages = [{"role": "system", "content": "S"}]
for _i in range(6):
    _sk2.messages += [{"role": "user", "content": f"q{_i}"},
                      {"role": "assistant", "content": "", "tool_calls": [{"id": f"h{_i}", "type": "function",
                       "function": {"name": "read_file", "arguments": "{}"}}]},
                      {"role": "tool", "tool_call_id": f"h{_i}", "tool_name": "read_file",
                       "content": "w" * (4 * (120_000 if _i >= 4 else 1000))}]   # newest 2 ~120k each
_sk2.ctx = lc.ContextMeter(_sk2)
_sk2_called = []
_sk2.compact = lambda target=None: _sk2_called.append(target)
_sk2._print_ctx = lambda: None
_b = _io_ck.StringIO()
with _ctx_ck.redirect_stdout(_b):
    lc.handle_compact_command(_sk2, _ckcfg, "100")
check("/compact <k>: huge recent results -> steps down to 0 kept whole, still no summary",
      not _sk2_called and "newest 0 kept whole" in _b.getvalue(), _b.getvalue())
# _trim_turn_to_budget: anchors alone too big -> None (caller drops the whole turn)
check("_trim_turn_to_budget: unshrinkable turn -> None",
      kb._trim_turn_to_budget([{"role": "user", "content": "ask"},
                               {"role": "assistant", "content": "X" * 80000}], 300) is None)
check("_trim_turn_to_budget: fits already -> returned as-is copy",
      kb._trim_turn_to_budget([{"role": "user", "content": "hi"},
                               {"role": "assistant", "content": "ok"}], 100000) is not None)
# _trim_turn_to_budget: NEVER orphan a tool_result from its tool_call. Regression for the
# wedge that 400'd a live session: a cut fell between assistant tool_call A and its result,
# leaving result-A at the head of the kept tail with no matching tool_use ("unexpected
# tool_use_id in tool_result blocks"). The cut must skip past any kept tool result.
_orph = [{"role": "user", "content": "do stuff"},
         {"role": "assistant", "content": "x" * 4000,
          "tool_calls": [{"id": "A", "function": {"name": "ssh", "arguments": {}}}]},
         {"role": "tool", "tool_call_id": "A", "tool_name": "ssh", "content": "R" * 4000},
         {"role": "assistant", "content": "more",
          "tool_calls": [{"id": "B", "function": {"name": "read", "arguments": {}}}]},
         {"role": "tool", "tool_call_id": "B", "tool_name": "read", "content": "b"},
         {"role": "assistant", "content": "final"}]
_ot = kb._trim_turn_to_budget(_orph, lc.messages_tokens(_orph, None) // 2) or []
_issued = {tc["id"] for m in _ot if m.get("role") == "assistant"
           for tc in (m.get("tool_calls") or [])}
check("_trim_turn_to_budget: never orphans a tool_result (API-400 wedge regression)",
      _ot and not [m for m in _ot if m.get("role") == "tool"
                   and m.get("tool_call_id") not in _issued])
...
cz.cfg = lc.Config(compact_soft=0.40, compact_at=0.60,
...
# 12e-bis. training-capture per-EXCHANGE outcome signal (STaR accept/reject).
import tempfile as _tf_cap, json as _json_cap
_capdir = _tf_cap.mkdtemp(prefix="lc-outcome-")
_ca = lc.Agent(lc.Config(cwd=FIX, capture_training=True, capture_training_dir=_capdir))
_ca.autosave_name = "outtest"
# followup classifier
check("outcome: classify correction", _ca._cap_classify_followup("no that's wrong, fix it") == "correction")
check("outcome: classify satisfied", _ca._cap_classify_followup("perfect, thanks") == "satisfied")
check("outcome: classify continuation (new task)", _ca._cap_classify_followup("now add a test for the parser") == "continuation")
check("outcome: correction beats satisfied", _ca._cap_classify_followup("thanks but this is broken") == "correction")
check("outcome: empty next turn -> continuation", _ca._cap_classify_followup("") == "continuation")
# accumulator roll-up: last exit code wins final_exit_ok; had_error ORs across the exchange
_ca._cap_new_run_turn("do the thing", False)
_ca._cap_turn = {"segments": [], "outcome_hints": {"any_tool_error": False}}
_ca._cap_add_result("run_command", "c1", {"text": "boom", "exit_code": 1, "error": None})
_ca._cap_add_result("run_command", "c2", {"text": "ok", "exit_code": 0})
check("outcome: had_error ORs a failed command even if a later one succeeds",
      _ca._cap_exchange["had_error"] is True)
check("outcome: final_exit_ok tracks the LAST exit-code-bearing command", _ca._cap_exchange["final_exit_ok"] is True)
# opening the next run_turn flushes the prior exchange, classifying THIS turn as its followup
_ca._cap_new_run_turn("nice, works", False)
_recs = [_json_cap.loads(l) for l in (Path(_capdir) / "outtest.jsonl").read_text().splitlines()]
_out = [r for r in _recs if r.get("kind") == "exchange_outcome"]
check("outcome: a sidecar exchange_outcome record is written on the next run_turn", len(_out) == 1)
check("outcome: sidecar has the right schema + run_turn_id",
      _out[0]["schema"] == "leancoder.training.outcome.v1" and _out[0]["run_turn_id"] == "outtest:1")
check("outcome: recovered-success recorded (final_exit_ok True, had_error True, followup satisfied)",
      _out[0]["final_exit_ok"] is True and _out[0]["exchange_had_error"] is True
      and _out[0]["user_followup"] == "satisfied")
# session-exit flush stamps the final open exchange as followup 'none'
_ca._cap_flush_exchange("none")
_recs2 = [_json_cap.loads(l) for l in (Path(_capdir) / "outtest.jsonl").read_text().splitlines()]
_out2 = [r for r in _recs2 if r.get("kind") == "exchange_outcome"]
check("outcome: exit flush closes the final exchange with followup 'none'",
      len(_out2) == 2 and _out2[1]["run_turn_id"] == "outtest:2" and _out2[1]["user_followup"] == "none")

# 12f. tool_timeout: the _dispatch wedge tripwire (in-process tools) + declarative opt-out
check("tool_timeout defaults to 60; run_command+background are UNBOUNDED_TOOLS",
      lc.Config().tool_timeout == 60
      and lc.UNBOUNDED_TOOLS == frozenset({"run_command", "background"}))
_tt = lc.Agent(lc.Config(cwd=FIX, tool_timeout=60))
# a plain core read tool gets the global ceiling
check("_tool_timeout_for: core read tool -> global default (60)",
      _tt._tool_timeout_for("read_file", None) == 60.0)
# core long-runners opt out
check("_tool_timeout_for: run_command/background -> None (own tier)",
      _tt._tool_timeout_for("run_command", None) is None
      and _tt._tool_timeout_for("background", None) is None)
# WRITE-tier core tools are NEVER abandoned (a mutation orphan could land late + race)
check("_tool_timeout_for: write-tier tools -> None (never abandon a mutation)",
      all(_tt._tool_timeout_for(_w, None) is None for _w in lc._WRITE_TIER)
      and "write_file" in lc._WRITE_TIER and "apply_diff" in lc._WRITE_TIER)
# lean-tool no_timeout opts out; lean-tool timeout overrides; plain lean-tool takes global
check("_tool_timeout_for: lean-tool no_timeout -> None, timeout=X -> X, plain -> global",
      _tt._tool_timeout_for("x", {"no_timeout": True}) is None
      and _tt._tool_timeout_for("x", {"no_timeout": False, "timeout": 5.0}) == 5.0
      and _tt._tool_timeout_for("x", {"no_timeout": False, "timeout": None}) == 60.0)
# tool_timeout=0 disables the tripwire entirely
_tt0 = lc.Agent(lc.Config(cwd=FIX, tool_timeout=0))
check("_tool_timeout_for: tool_timeout=0 disables (everything unbounded)",
      _tt0._tool_timeout_for("read_file", None) is None)
# MCP calls opt out (own transport timeout)
check("_tool_timeout_for: MCP-namespaced call -> None",
      _tt._tool_timeout_for(lc.MCP_NS + "srv__tool", None) is None)
# a genuinely slow in-process tool trips the cap and hands control back with a message
_ttx = lc.Agent(lc.Config(cwd=FIX, tool_timeout=1))
_ttx.lean_tools.lean_tools["_slowtool"] = {
    "run": (lambda a, c: __import__("time").sleep(10) or "done"),
    "safe": True, "driver_only": False, "no_timeout": False, "timeout": None}
import time as _t
_t0 = _t.time(); _slowres = _ttx._dispatch("_slowtool", {}); _elapsed = _t.time() - _t0
check("slow in-process tool trips tool_timeout ~1s (not 10s) and returns a wedge message",
      _elapsed < 4 and "exceeded" in _slowres and "wedge timeout" in _slowres)
# a fast tool under the cap returns its real result through the thread path
_ttx.lean_tools.lean_tools["_fasttool"] = {
    "run": (lambda a, c: "quick"), "safe": True, "driver_only": False,
    "no_timeout": False, "timeout": None}
check("fast in-process tool returns its real result through the bounded path",
      _ttx._dispatch("_fasttool", {}) == "quick")
...
# CHAT-MODE system prompt: leash=chat -> NO tools -> serve the lean chat prompt, NOT the
# agentic one. The tool scaffolding (<decision_format>/<batch_reads>/<ask_operator>) is
# dead machinery with no tools and a small model parrots it ("DECISION 1/2", "enable the
# tools for /leash rwe"). Regression for that bug report.
_chat = lc.Agent(lc.Config(leash="chat"))
_cs = _chat._system()
check("chat leash: no tools attached", _chat.tool_defs == [])
check("chat leash: serves the lean chat prompt", "CHAT-ONLY mode" in _cs)
check("chat leash: strips <decision_format> tool scaffolding", "<decision_format>" not in _cs)
check("chat leash: strips <batch_reads>", "<batch_reads>" not in _cs)
check("chat leash: strips <ask_operator>", "<ask_operator>" not in _cs)
check("chat leash: no 'TOOL USE'/'CALL THE TOOL' addendum", "CALL THE TOOL" not in _cs)
# rwe leash keeps the full agentic prompt
_rwe = lc.Agent(lc.Config(leash="rwe"))
check("rwe leash: full agentic prompt", "<decision_format>" in _rwe._system() and _rwe.tool_defs)
# live /leash toggle rebuilds messages[0] the RIGHT way (tools refreshed BEFORE _system):
# rwe->chat drops scaffolding, chat->rwe restores it (ordering regression).
lc._apply_leash(_rwe, _rwe.cfg, "chat", persist=False)
check("/leash rwe->chat: chat prompt live in messages[0]",
      "CHAT-ONLY mode" in _rwe.messages[0]["content"] and _rwe.tool_defs == [])
lc._apply_leash(_rwe, _rwe.cfg, "rwe", persist=False)
check("/leash chat->rwe: agentic prompt restored in messages[0]",
      "<decision_format>" in _rwe.messages[0]["content"] and _rwe.tool_defs)
...
# ONE compaction prompt (manual + auto + elected all read 'compact'); nudge is separate.
check("compact is a builtin prompt", "compact" in lc.BUILTIN_PROMPTS)
check("compact_nudge is a builtin prompt", "compact_nudge" in lc.BUILTIN_PROMPTS)
check("auto_compact is NO LONGER a separate builtin prompt (merged into 'compact')",
      "auto_compact" not in lc.BUILTIN_PROMPTS)
check("AUTO_COMPACT_INSTR is now an alias of COMPACT_INSTR (one source, can't drift)",
      lc.AUTO_COMPACT_INSTR is lc.COMPACT_INSTR)
# guard the fixed self-contradiction (llm-tester report): the prompt must NOT carry a
# blanket "no tool call" ban (it also asks to save durable notes, which needs a tool call).
# The durable-save step must come BEFORE the blocks; the STOP clause is about not
# tool-calling AFTER the blocks, not a global ban.
_aci = lc.COMPACT_INSTR
check("compact prompt: no blanket 'no tool call' ban (was contradictory)",
      "no tool call" not in _aci)
check("compact prompt: durable-save is invited before the blocks",
      "save it now with your tools" in _aci
      and _aci.index("save it now") < _aci.index("START with"))
...
# ==========================================================================
# SECTION: Ollama num_ctx, host failover, per-host models, machines
# ==========================================================================
...
try:
    ...
    with contextlib.redirect_stdout(_b):
        _fo_ok = olm._failover(fa, fc, announce=True)   # NOT `ok`: that's the global gate flag
    check("_failover: jumps to next live host", _fo_ok is True and fc.host == "http://C:11434", fc.host)
...
# MRU ordering (like /load): a more-recently-used saved target floats to the top; the
# numeric arg still selects exactly the row the picker renders (one builder).
_mru = lc._connect_menu_order(
    lc.Config(connect_hosts={"box1": "user@192.0.2.1", "box2": "user@192.0.2.2"},
              connect_used={"user@192.0.2.2": 2000, "user@192.0.2.1": 1000}),
    _FakeAg(["user@open"]))
check("connect menu order: most-recently-used first",
      _mru == ["user@192.0.2.2", "user@192.0.2.1", "user@open"])
check("connect menu MRU: numeric arg picks the same row menu shows",
      lc.menu_resolve("1", _mru) == "user@192.0.2.2")
check("connect menu order: no history -> config order (unchanged)",
      lc._connect_menu_order(
          lc.Config(connect_hosts={"box1": "user@192.0.2.1", "box2": "user@192.0.2.2"}),
          _FakeAg([])) == ["user@192.0.2.1", "user@192.0.2.2"])
...
# ==========================================================================
# SECTION: Remote executor + SSH transport + RemoteWorkspace + Windows
# ==========================================================================
...
try:
    ...
    check("executor handshake reports detach (POSIX: bg tasks survive an ssh drop)",
          client.ready.get("detach") is True)
...
# mid-stream SSE overload must be RETRIED like an HTTP 529, not surface raw + die.
# anthropic_plan: transient error frame -> 'API 529:' shape the retry classifier catches.
def _plan_sse_retryable(etype):
    try:
        _plan_m._raise_sse_error({"type": etype, "message": "Overloaded"})
        return None
    except RuntimeError as _e:
        _s = str(_e)
        return (_plan_m._status_from_err(_s) in _plan_m._RETRY_CODES
                or _plan_m._is_transient_err(_s))
try:
    _plan_m = _il.import_module("providers.anthropic_plan")
    check("anthropic_plan: mid-stream overloaded_error SSE frame is retryable (was raw -> die)",
          _plan_sse_retryable("overloaded_error") is True)
    check("anthropic_plan: mid-stream api_error SSE frame is retryable",
          _plan_sse_retryable("api_error") is True)
    check("anthropic_plan: a NON-transient SSE error (invalid_request_error) still raises raw",
          _plan_sse_retryable("invalid_request_error") is False)
except ModuleNotFoundError:
    pass  # anthropic_plan not bundled publicly; skip where absent
# anthropic_api: an EMPTY reply is retried like a transient (same backoff budget, same
# request), the model's history is never touched, and the operator is told only after the
# retries are spent - neutrally (no "REFUSED" verdict).
import io as _io_er, contextlib as _ctx_er
_er_sleep = _api_m._interruptible_sleep
_api_m._interruptible_sleep = lambda secs, should_abort: True     # no real waiting in tests
_erc = _api_m._ApiKeyClient(lc.Config(model="m"))
_seq_er = [({"role": "assistant", "content": ""}, 10, False)] * 2 + \
          [({"role": "assistant", "content": "hello"}, 10, False)]
_calls_er = []
def _fake_send(key, payload, should_abort):
    _calls_er.append(1); _erc.last_stop_reason = "refusal" if len(_calls_er) < 3 else "end_turn"
    return _seq_er[len(_calls_er) - 1]
_erc._send = _fake_send
_b = _io_er.StringIO()
with _ctx_er.redirect_stdout(_b):
    _got = _erc._send_retrying_empty("k", {"model": "m"}, None)
check("anthropic_api: empty reply is re-sent (transient), then the real reply returns",
      _got[0]["content"] == "hello" and len(_calls_er) == 3 and _erc.last_empty_retries == 2)
check("anthropic_api: a recovered empty reply prints no final 'no reply' notice",
      "possibly a refusal" not in _b.getvalue(), _b.getvalue())
_calls_er.clear(); _seq_er = [({"role": "assistant", "content": ""}, 10, False)] * 10
def _fake_send2(key, payload, should_abort):
    _calls_er.append(1); _erc.last_stop_reason = "refusal"; return _seq_er[0]
_erc._send = _fake_send2
_b = _io_er.StringIO()
with _ctx_er.redirect_stdout(_b):
    _got = _erc._send_retrying_empty("k", {"model": "m"}, None)
check("anthropic_api: empty reply retried up to the SAME budget as errors, then returned",
      len(_calls_er) == _api_m._MAX_RETRIES + 1 and _got[0]["content"] == "")
check("anthropic_api: exhausted empty -> neutral operator notice (facts, no verdict)",
      f"no reply after {_api_m._MAX_RETRIES} retries (stop_reason: refusal) - possibly a refusal, "
      "API congestion, or a network glitch" in _b.getvalue() and "REFUSED" not in _b.getvalue(),
      _b.getvalue()[-240:])
check("anthropic_api: the empty reply handed back carries NO note/stop_reason (model never told)",
      set(_got[0]) == {"role", "content"})
_api_m._interruptible_sleep = _er_sleep
# anthropic_api: transient SSE frame -> _TransientStreamError (its retry loop catches it).
def _api_sse_kind(etype):
    try:
        _api_m._raise_sse_error({"type": etype, "message": "Overloaded"})
        return None
    except _api_m._TransientStreamError:
        return "transient"
    except RuntimeError:
        return "raw"
check("anthropic_api: mid-stream overload SSE -> _TransientStreamError (retried)",
      _api_sse_kind("overloaded_error") == "transient")
check("anthropic_api: non-transient SSE error -> plain RuntimeError (not retried)",
      _api_sse_kind("invalid_request_error") == "raw")

# mid-stream IDLE stall (socket went silent) must bail to the bounded retry path,
# not hang the turn. The node-path helper raises a 'request failed:' idle-stall
# error; it must classify as transient so _send_with_retry backs off + retries.
try:
    check("anthropic_plan: idle-stall 'request failed:' error is transient (retried, not raw)",
          _plan_m._is_transient_err(
              f"request failed: stream idle >{_plan_m._STREAM_IDLE_TIMEOUT}s "
              f"(stalled mid-stream)") is True)
    check("anthropic_plan: stream idle timeout armed (~90s, not disabled/harsh)",
          45 <= _plan_m._STREAM_IDLE_TIMEOUT <= 180)
except (NameError, AttributeError):
    pass  # anthropic_plan not bundled publicly; skip where absent
check("anthropic_api: stream idle timeout armed (~90s, not disabled/harsh)",
      45 <= _api_m._STREAM_IDLE_TIMEOUT <= 180)
# stream_tiered's decode-phase stall raises StreamStall, a ConnectionError, so the
# providers' transport-retry branches (which catch ConnectionError / translate to
# 'request failed:') pick it up rather than it escaping as a fatal.
check("core: StreamStall is a ConnectionError (folds into transport retry)",
      issubclass(lc.StreamStall, ConnectionError))
check("core: stream_tiered accepts an explicit idle= override (provider arms 90s)",
      "idle" in _insp.signature(lc.stream_tiered).parameters)
...
# approval="ask" explicitly: these exercise the per-call confirm/prompt path (default is
# now "session", which caches an approval and would skip the second prompt).
c0 = lc.Config(approval="ask")
...
check("session-env names the host tools run on (the remote)", "\nhost: gpu\n" in agm._session_env())
agm.set_remote(None)
check("session-env cwd returns to local on disconnect", str(FIX) in agm._session_env())
check("session-env host switches back to this box on disconnect",
      f"\nhost: {lc._HOSTNAME}\n" in agm._session_env())

# remote lost (tool call or /sh): ONE path - drop to local, neutral model line
class _LostWS(_WS):
    host = "user@192.0.2.37"; ctl = "/nonexistent"
    def connect(self, batch=False): raise ConnectionError("ssh could not connect")
    def close(self): pass
agd = lc.Agent(lc.Config(cwd=FIX)); _lw = _LostWS()
agd.remotes[_lw.host] = _lw; agd.set_remote(_lw)
import io as _io_rl, contextlib as _ctx_rl
with _ctx_rl.redirect_stdout(_io_rl.StringIO()):
    _msg = agd._on_remote_lost(_lw.host, "ssh could not connect")
check("remote lost: drops to local (glyph follows agent.remote)", agd.remote is None)
check("remote lost: neutral model line names where it is now",
      _msg == f"remote {_lw.host} dropped (ssh could not connect); the session is now "
              f"running LOCALLY on {lc._HOSTNAME}.", _msg)
# /sh: exit 255 + failed batch reconnect -> the same drop; any other exit (or a 255
# whose reconnect works = the command's own code) -> stays connected
agd.remotes[_lw.host] = _lw; agd.set_remote(_lw)
with _ctx_rl.redirect_stdout(_io_rl.StringIO()):
    lc._sh_check_remote(agd, _lw, "[operator ran a command directly] exit 1")
check("/sh: a non-255 exit never drops the remote", agd.remote is _lw)
class _OkWS(_LostWS):
    def connect(self, batch=False): return self
_ok = _OkWS(); agd.remotes[_ok.host] = _ok; agd.set_remote(_ok)
with _ctx_rl.redirect_stdout(_io_rl.StringIO()):
    lc._sh_check_remote(agd, _ok, "[operator dropped into an interactive shell] (last exit 255)")
check("/sh: exit 255 but the reconnect works -> command's own 255, stays remote", agd.remote is _ok)
agd.remotes[_lw.host] = _lw; agd.set_remote(_lw)
with _ctx_rl.redirect_stdout(_io_rl.StringIO()):
    lc._sh_check_remote(agd, _lw, "ssh: connect to host 192.0.2.37 port 22: unreachable (last exit 255)")
check("/sh: exit 255 + reconnect fails -> the same drop to local", agd.remote is None)
...
agd = lc.Agent(lc.Config(cwd=FIX, approval="ask", lean_tools_dir=str(plugdir), lean_tools_enabled=["danger"]))
_oap = lc._ask
lc._ask = lambda q: False
with contextlib.redirect_stdout(io.StringIO()):
...
# /prompt new vs edit: honest existence gating. 'new' refuses an existing name (no
# clobber), 'edit' refuses a missing one (nothing to edit); the file state is the proof.
_pnag = type("Ag", (), {"cfg": lc.Config(model="m"), "_refresh_system_prompt": None})()
lc._refresh_system_prompt = lambda *a, **k: None
(lc.PROMPTS_DIR / "existing.txt").write_text("already here")
_before = (lc.PROMPTS_DIR / "existing.txt").read_text()
lc._edit_prompt_file(_pnag, _pnag.cfg, "existing", create=True)   # new on existing -> refuse
check("/prompt new: refuses an existing name (no clobber)",
      (lc.PROMPTS_DIR / "existing.txt").read_text() == _before)
lc._edit_prompt_file(_pnag, _pnag.cfg, "ghost", create=False)     # edit on missing -> refuse
check("/prompt edit: refuses a missing name (does not seed a file)",
      not (lc.PROMPTS_DIR / "ghost.txt").is_file())
check("argcomp: /prompt offers the 'new' verb",
      "new" in lc._arg_completions(None, None, "/prompt"))
...
try:
    ...
    with contextlib.redirect_stdout(io.StringIO()) as _pb:
        lc.confirm_remote_tool(lc.Config(approval="ask"), ws.host, "apply_diff",
            {"path": "big.txt",
             "diff": "<<<<<<< SEARCH\nMARKER_PAST_6K\n=======\nMARKER_REPLACED\n>>>>>>> REPLACE"},
            fetch=ws.read_raw)
    lc._ask = _oa2
...
# blockquote: '>' markers stripped, rendered as PLAIN prose with NO left bar glyph
# (copy-targeted content must not be contaminated by a per-line bar on drag-select)
_strip_ansi = lambda s: lc._ANSI_RE.sub("", s)
check("style_md_line strips '>' quote marker and adds NO bar glyph",
      lc.style_md_line("> hello there") == lc.dim("hello there")
      and lc.GLYPH["userbar"] not in lc.style_md_line("> hello there"))
check("style_md_line strips nested '>>' quote markers, still no bar",
      lc.GLYPH["userbar"] not in lc.style_md_line(">> deep")
      and ">" not in _strip_ansi(lc.style_md_line(">> deep")))
# fenced code INSIDE a blockquote ("> ```") - the model's "message to send" with
# copy commands. The '>' must not defeat fence detection; code emits verbatim, barless.
_qbuf = []
_qs = lc.MarkdownStream(_qbuf.append)
_qs.feed("> Your v2 is live\n> ```\n> git fetch origin\n> git checkout v2\n> ```\n")
_qs.flush()
_qout = _strip_ansi("".join(_qbuf))
check("MarkdownStream: quoted fenced code is verbatim, no bar, no '>', no fence",
      "git fetch origin" in _qout and "git checkout v2" in _qout
      and lc.GLYPH["userbar"] not in _qout and ">" not in _qout and "```" not in _qout)
...
# ==========================================================================
# SECTION: Session lifecycle: autosave, snapshots, statusline
# ==========================================================================
...
# --- steal detection + detach-on-steal (idle-boundary early fork) ------------
# A session whose lock we OWN is not stolen; one whose lock a live OTHER instance
# holds is. _detach_if_stolen renames us to a fresh auto- name (user-only notice).
_stag = lc.Agent(lc.Config(cwd=FIX, approval="auto", autosave=True))
_stag.messages = [{"role": "system", "content": "S"},
                  {"role": "user", "content": "hi"}]
_stag.autosave_name = lc._new_autosave_name()
lc._write_lock(_stag.autosave_name)   # we own it
check("_session_stolen: false when we hold our own lock",
      not lc._session_stolen(_stag, _stag.cfg))
check("_detach_if_stolen: no-op (returns False) when we own the lock",
      lc._detach_if_stolen(_stag, _stag.cfg) is False)
# simulate another LIVE instance (this-host, an alive pid != ours) claiming it
import os as _os_st
lc._lock_path(_stag.autosave_name).write_text(__import__("json").dumps(
    {"pid": _os_st.getppid(), "host": lc._HOSTNAME, "ts": lc.time.time()}))
check("_session_stolen: true when a live OTHER instance holds the lock",
      lc._session_stolen(_stag, _stag.cfg))
_stolen_from = _stag.autosave_name
with _ctx_h.redirect_stdout(_io_h.StringIO()):
    _detached = lc._detach_if_stolen(_stag, _stag.cfg)
check("_detach_if_stolen: renames to a fresh auto- name on a steal",
      _detached is True and _stag.autosave_name != _stolen_from
      and _stag.autosave_name.startswith(lc.AUTOSAVE_PREFIX))
# incognito / autosave-off never detaches (no lock is ours to lose)
_stag.cfg.incognito = True
check("_session_stolen: false under incognito", not lc._session_stolen(_stag, _stag.cfg))
...
# _session_live_worker: a board-spawned worker (from_session=<name>) drives a session
# WITHOUT holding its .lock; a REPL opening it must still be able to detect that.
_slw_reg_save = lc._bg_load()                     # snapshot; restore after (records below leak)
_swname = "workerdriven"
lc._session_path(_swname).write_text(lc.json.dumps({"meta": {}, "messages": []}))
check("_session_live_worker: None when no worker drives the session",
      lc._session_live_worker(_swname) is None)
lc._bg_register(lc.os.getppid(), "lean_coder --agent-run x", _exlog + ".swkr",
                kind="worker", meta={"from_session": _swname})
check("_session_live_worker: finds a live worker spawned from the session file",
      lc._session_live_worker(_swname) == lc.os.getppid())
check("_session_live_worker: a plain (non-worker) bg record doesn't count",
      lc._session_live_worker("nosuchsess") is None)
# a DEAD worker pid frees it (same pid-liveness rule as the lock)
lc._bg_register(999999, "lean_coder --agent-run y", _exlog + ".dwkr",
                kind="worker", meta={"from_session": "deadwkr"})
lc._session_path("deadwkr").write_text(lc.json.dumps({"meta": {}, "messages": []}))
check("_session_live_worker: a dead worker pid does not count as driving",
      lc._session_live_worker("deadwkr") is None)
lc._session_path(_swname).unlink(missing_ok=True)
lc._session_path("deadwkr").unlink(missing_ok=True)
_bg_restore(_slw_reg_save)                         # restore registry (drop the test worker recs)
# _prune_dead_tmp_writes: a crash mid-atomic-write leaves '<f>.tmp<pid>'. Dead writer's
# go; a live writer's (mid-save in another session) and the real file stay.
_tw_dead = subprocess.Popen(["true"]); _tw_dead.wait()
(lc.SESSIONS_DIR / f"x.json.tmp{_tw_dead.pid}").write_text("half")
(lc.SESSIONS_DIR / f"y.json.tmp{lc.os.getppid()}").write_text("mid-save")
(lc.SESSIONS_DIR / "x.json").write_text("{}")
lc._prune_dead_tmp_writes()
check("tmp-write prune: dead writer's leftover removed",
      not (lc.SESSIONS_DIR / f"x.json.tmp{_tw_dead.pid}").exists())
check("tmp-write prune: live writer's temp + the real file kept (liveness, not age)",
      (lc.SESSIONS_DIR / f"y.json.tmp{lc.os.getppid()}").exists() and (lc.SESSIONS_DIR / "x.json").exists())
for _n in (f"y.json.tmp{lc.os.getppid()}", "x.json"):
    (lc.SESSIONS_DIR / _n).unlink(missing_ok=True)
...
# ==========================================================================
# SECTION: Composer: input buffer, queue, paste/key parsing
# ==========================================================================
...
# remote flag swaps the prompt glyph (local '>' vs remote '>>') on the input row.
_cr = lc.Composer(); _cr.feed("x"); _cr.remote = True
check("composer: remote=True swaps to the remote prompt glyph",
      _cr.input_text() == lc.GLYPH["prompt_remote"] + " x")
_cr.remote = False
check("composer: remote=False shows the local prompt glyph",
      _cr.input_text() == lc.GLYPH["prompt"] + " x")
_c.feed("\x7f")
...
# _may_prompt_on_tty: a LOCAL ssh/git run_command gets the composer suspended so a
# cold-cache pinentry draws on a clean tty instead of colliding with cbreak mode.
check("_may_prompt_on_tty: bare ssh", lc._may_prompt_on_tty("ssh user@192.0.2.7 whoami"))
check("_may_prompt_on_tty: git fetch (ssh remote)", lc._may_prompt_on_tty("cd ~/x && git fetch origin"))
check("_may_prompt_on_tty: scp/sftp/rsync", lc._may_prompt_on_tty("scp a user@h:/b")
      and lc._may_prompt_on_tty("rsync -a x user@h:/y"))
check("_may_prompt_on_tty: ssh after a pipe/semicolon", lc._may_prompt_on_tty("echo hi; ssh h w")
      and lc._may_prompt_on_tty("true && ssh h w"))
check("_may_prompt_on_tty: false on plain commands (keeps the spinner)",
      not lc._may_prompt_on_tty("ls -la") and not lc._may_prompt_on_tty("cat sshd_config")
      and not lc._may_prompt_on_tty("python3 build.py"))
...
class _FakeAg:
    def run_turn(self, x, is_model_generated=False): return ("ran", is_model_generated)
...
# the wrapper MUST forward extra args/kwargs (e.g. is_model_generated from the
# autostart-after-compact call) - a wrapper that hardcodes (self, user_input) crashes
# _consume_autostart. Regression guard for the v0.10.10 arity bug.
check("notify: run_turn wrapper forwards is_model_generated kwarg",
      _FakeAg().run_turn("x", is_model_generated=True) == ("ran", True))
...
# REGRESSION: on quit, the final autosave MUST run BEFORE close_all_remotes, so the
# session records the remote host it ran on (and resumes reconnecting) - not remote=None
# (which resumed LOCAL). Mirrors the exit-path ordering in repl()/handle_quit_command.
_qdir = lc.SESSIONS_DIR
lc.SESSIONS_DIR = Path(tempfile.mkdtemp(prefix="lc_quit_"))
try:
    _qag = lc.Agent(lc.Config(cwd=FIX, approval="auto", autosave=True))
    _qag.messages = [{"role": "system", "content": "S"},
                     {"role": "user", "content": "ran on a remote"}]
    _qag.set_remote(_FakeWS("user@examplebox"))
    lc.autosave_session(_qag, _qag.cfg)   # exit order: save THEN close
    _qag.close_all_remotes()
    _qmeta = lc.load_session(_qag.autosave_name)["meta"]
    check("quit: final autosave captures the remote host (resumes reconnecting)",
          _qmeta.get("remote") == "user@examplebox")
    # and prove the BROKEN order (close first) would have lost it - guards the ordering
    _qag2 = lc.Agent(lc.Config(cwd=FIX, approval="auto", autosave=True))
    _qag2.messages = [{"role": "system", "content": "S"},
                      {"role": "user", "content": "ran on a remote"}]
    _qag2.set_remote(_FakeWS("user@examplebox"))
    _qag2.close_all_remotes()             # WRONG order: nulls remote first
    lc.autosave_session(_qag2, _qag2.cfg)
    check("quit: (control) saving AFTER close loses the remote -> remote=None",
          lc.load_session(_qag2.autosave_name)["meta"].get("remote") is None)
finally:
    shutil.rmtree(lc.SESSIONS_DIR, ignore_errors=True)
    lc.SESSIONS_DIR = _qdir
...
# ==========================================================================
# SECTION: Version/update precedence + provider registration API
# ==========================================================================
...
# release notes: shown on UPDATE (last_seen older), silent on fresh install + when current
check("release notes: fresh install (blank marker) shows nothing",
      lc._release_notes_since("") == [])
check("release notes: up-to-date shows nothing",
      lc._release_notes_since(lc.__version__) == [])
check("release notes: an update from an older build surfaces this version",
      lc.__version__ in [v for v, _ in lc._release_notes_since("0.0.1")])
check("release notes: every RELEASE_NOTES version is <= this build (no notes from the future)",
      all(lc.version_tuple(v) <= lc.version_tuple() for v in lc.RELEASE_NOTES))
...
# _wk_daily lives in the private anthropic_plan provider (via `be`), so guard on it -
# a public checkout without the local suite skips these two checks cleanly.
if _be:
    _d7 = _be._wk_daily(90, (lc.datetime.datetime.now(lc.datetime.timezone.utc)
    ...
    _d1 = _be._wk_daily(5, (lc.datetime.datetime.now(lc.datetime.timezone.utc)
...
check("_fmt_reset: under an hour shows HH:MM(Nm)",
      ":" in _r_mins and _r_mins.endswith("m)") and ">" not in _r_mins)
...
try:
    ...
    class _FakeChatClient:
        def __init__(self): self.last_out_tokens = None; self.last_eval_ns = None
        def chat(self, messages, tools, should_abort=None):
            self.last_out_tokens = 20
            self.last_eval_ns = 1_000_000_000   # 20 tok in 1s -> 20 tok/s
    ...
    check("session_eval accumulates decode tokens+ns",
          tag.session_eval_tokens == 20 and tag.session_eval_ns == 1_000_000_000)
    check("decode_tok_s computes honest tok/s", tag.decode_tok_s() == 20.0)
    tag.run_turn("again")
    ...
    check("reset zeroes decode telemetry + decode_tok_s None",
          tag.session_eval_tokens == 0 and tag.session_eval_ns == 0 and tag.decode_tok_s() is None)
    # a provider that reports tokens but NO gen-time (API backend) contributes nothing
    class _NoTimingClient:
        ...
        def list_models(self): return []
        def running_models(self): return []
    tag2 = lc.Agent(lc.Config(cwd=FIX, model="qwen")); tag2.client = _NoTimingClient()
    tag2.run_turn("hello")
    check("no gen-time -> session_out counts but decode_tok_s stays None",
          tag2.session_out == 20 and tag2.session_eval_tokens == 0 and tag2.decode_tok_s() is None)
...
for _pname, _ptag in (("anthropic_api", "api"), ("gemini", "gem"),
    ...
    # Unconfigured, a provider falls back to the REAL ~/.config/leancoder: with the user's
    # real key there, list_models() would hit the network and overwrite their model cache.
    # Point it at an empty scratch dir so "unauthed" really is unauthed + writes nothing.
    if hasattr(_pp, "_CONFIG_DIR"):
        _pp._CONFIG_DIR = Path(tempfile.mkdtemp(prefix=f"lc_prov_{_pname}_"))
    _pd = _pp.PROVIDER
...
# ============================================================================
# ==========================================================================
# SECTION: Session modes + CLI arg completion + /set + leash + per-model
# ==========================================================================
...
try:
    ...
    # per-worker tool allowlist (cfg.tool_allowlist): narrows the surface, keeps meta
    check("tool_allowlist None (default) -> full surface unchanged",
          _names(lc.Config(leash="rwe")) == _names(lc.Config(leash="rwe", tool_allowlist=None)))
    _aw = _names(lc.Config(leash="rwe", tool_allowlist=("read_file", "run_command")))
    check("tool_allowlist narrows to listed tools", set(_aw) & {"write_file", "apply_diff"} == set()
          and "read_file" in _aw and "run_command" in _aw)
    check("tool_allowlist always keeps meta tools", "update_plan" in _aw and "note" in _aw)
    _awl = [t["function"]["name"] for t in lc.active_tools(
              lc.Config(leash="rwe", tool_allowlist=("peek",)), lean_tool_schemas=[_safe_t, _unsafe_t])]
    check("tool_allowlist filters lean-tools too", "peek" in _awl and "zap" not in _awl)
    _awle = _names(lc.Config(leash="r", tool_allowlist=("read_file", "write_file")))
    check("tool_allowlist still leash-capped (write dropped at r)",
          "read_file" in _awle and "write_file" not in _awle)
    ...
    # (The chat-mode system prompt DOES state "no tools" itself - that's the fix for the
    # small-model parroting bug - so we assert the <leash> NOTE specifically stays out.)
    check("leash chat: model told via pending note, NOT baked into system prompt",
          "no tools" in (_mag._pending_ai_note or "")
          and "<leash>" in (_mag._pending_ai_note or "")
          and "<leash>" not in _mag.messages[0]["content"])
    with _ctxl.redirect_stdout(_io.StringIO()):
    ...
    # is written and reloaded so every new/auto-spawned session keeps it; the default
    # "session" is omitted; junk in the file falls back to "ask".
    lc.save_config(lc.Config(model="m", approval="auto"), quiet=True)
    check("save: non-default approval persisted", 'approval = "auto"' in _mc2.read_text())
    lc.save_config(lc.Config(model="m"), quiet=True)
    check("save: default approval (session) omitted", "approval" not in _mc2.read_text())
finally:
...
check("chat-only model: session-env tail tells the model it has no tools",
      "NO tools" in _csa._session_env() and "chat-only" in _csa._session_env())
check("chat-only model: system prompt is the lean chat prompt (tool scaffolding stripped)",
      "CHAT-ONLY mode" in _csa._system() and "<decision_format>" not in _csa._system())
import re as _re_cs
...
# ============================================================================
# ==========================================================================
# SECTION: Grouped lean-tools + provider/tool discovery dirs
# ==========================================================================
...
# release-notes-on-/update: parse RELEASE_NOTES out of a lean_coder.py ON DISK (no import,
# since the running process is still the old build) and pick the range shown.
(_usrc / "lean_coder.py").write_text(
    'RELEASE_NOTES = {"1.0.0": ["big thing"], "1.1.0": ["another"]}\n')
_pn = _upd._notes_from_source(_usrc / "lean_coder.py")
check("update notes: parse RELEASE_NOTES from source via ast (no import)",
      _pn == {"1.0.0": ["big thing"], "1.1.0": ["another"]}, str(_pn))
check("update notes: missing/garbage source -> {} (never raises)",
      _upd._notes_from_source(_usrc / "nope.py") == {})
_ubuf = _uio2.StringIO()
with _uctx.redirect_stdout(_ubuf):
    _upd._show_update_notes(_usrc, "0.9.9", "1.1.0", lc.version_tuple,
                            lc.bold, lc.green, lc.dim, "*")
_uout = _ubuf.getvalue()
check("update notes: shows only versions in (from, to] range",
      "big thing" in _uout and "another" in _uout and "what's new" in _uout, _uout[:80])
_ubuf2 = _uio2.StringIO()
with _uctx.redirect_stdout(_ubuf2):
    _upd._show_update_notes(_usrc, "1.1.0", "1.1.0", lc.version_tuple,
                            lc.bold, lc.green, lc.dim, "*")
check("update notes: nothing when already at target (empty range)", _ubuf2.getvalue() == "")
...
# REVERSE orphan (the mg wedge): a tool_result whose tool_call was dropped by a
# compaction/window cut. API: 'unexpected tool_use_id in tool_result blocks' every
# turn. repair must DROP the orphan result so a resumed/edited session can send.
rev = [
    {"role": "user", "content": "compacted summary"},
    {"role": "user", "content": "hi"},
    {"role": "tool", "tool_name": "ssh", "tool_call_id": "toolu_orphan", "content": "x"},
    {"role": "assistant", "content": "ok"},
]
rf = lc.repair_tool_pairs(rev)
check("repair drops a reverse-orphan tool_result (API-400 resume wedge)",
      not any(m.get("role") == "tool" for m in rf)
      and [m["role"] for m in rf] == ["user", "user", "assistant"])
...
# ==========================================================================
# SECTION: Per-process run dirs (liveness reaper, no age)
# ==========================================================================
import tempfile as _tf_rd, subprocess as _sp_rd
_rd_root = Path(_tf_rd.mkdtemp())
_rd_save = lc.run_dir
lc.run_dir = lambda: _rd_root
try:
    _dead = _sp_rd.Popen(["true"]); _dead.wait()          # a pid that is certainly gone
    _mine = lc._run_pid_dir(); _mine.mkdir()
    _live = _rd_root / f"{lc._HOSTNAME}-1"; _live.mkdir()      # pid 1: always alive
    _gone = lc._run_pid_dir(_dead.pid); _gone.mkdir()
    _other = _rd_root / f"otherbox-{_dead.pid}"; _other.mkdir()  # foreign host: never ours
    _dashy = _rd_root / f"my-dashed-host-{_dead.pid}"; _dashy.mkdir()
    _stray = _rd_root / "notapid"; _stray.mkdir()
    _got = set(lc._run_dead_dirs())
    check("run dirs: dead same-host pid dir is reaped", _gone in _got, str(_got))
    check("run dirs: own + live pid dirs kept (liveness, never age)",
          _mine not in _got and _live not in _got)
    check("run dirs: foreign-host / dashed-foreign / non-pid dirs never touched",
          not ({_other, _dashy, _stray} & _got), str(_got))
    check("run dirs: own dir named <host>-<pid>",
          _mine.name == f"{lc._HOSTNAME}-{os.getpid()}")
finally:
    lc.run_dir = _rd_save
    shutil.rmtree(_rd_root, ignore_errors=True)
check("run_dir() resolves + is writable", lc.run_dir().is_dir() and os.access(lc.run_dir(), os.W_OK))

shutil.rmtree(FIX, ignore_errors=True)

# ==========================================================================
# SECTION: Hygiene sweep + FILE MAP drift gate (final gates)
# ==========================================================================
...
# from the live banners and assert every entry is present. Covers BOTH single-file
# modules the generator maintains: lean_coder.py and this harness itself.
_repo = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
for _rel in ("lean_coder.py", os.path.join("tests", "_smoketest.py")):
    _src = open(os.path.join(_repo, _rel)).readlines()
    _expected = [f"  L{i + 1:<6} {ln[len('# SECTION:'):].strip()}"
                 for i, ln in enumerate(_src) if ln.startswith("# SECTION:")]
    _map_txt = "".join(_src)
    if "=== FILE MAP" in _map_txt:
        ...
        check(f"FILE MAP index matches SECTION banners in {_rel} (run tools/gen_section_index.py)",
              not _stale, ("\n    stale/missing: " + "; ".join(e.strip() for e in _stale[:6])) if _stale else "")
    else:
        check(f"FILE MAP index present in {_rel}", False, "no '=== FILE MAP' block in module docstring")
