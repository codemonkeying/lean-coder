...
BUILTIN_TOOLS = [
    ...
    {"name": "run_command", "tier": "exec",
     "description": "Run a non-interactive shell command (builds, tests, git, file ops); returns exit code + stdout/stderr (truncated - if clipped, pipe to a file or use grep/head/tail to read specific parts). Fresh shell each call - cd/env don't persist, chain with '&&'. FOREGROUND runs are killed at the configured timeout ({command_timeout}s; set via /settings command_timeout) and you get nothing back but a timeout error - so anything that may run longer, or any server/daemon/watcher, MUST be backgrounded: end the command with ' &' and it returns a pid immediately (poll with bg_status; tune with notify_on_exit/heartbeat_timeout/max_runtime). LIFETIME: backgrounded jobs (and dispatched workers) live ONLY while this lean-coder session runs and die on session exit; for something that must outlive the session, launch it OS-level (nohup/systemd) directly, NOT via ' &'. For sudo/password/interactive use ask_user_to_run; for a shell that stays open (REPL, ssh) use shell_session.",
     "parameters": {"type": "object",
        "properties": {"cmd": {"type": "string", "description": "The shell command to run. End with ' &' to background it."},
                       "notify_on_exit": {"type": "boolean", "description": "Bg jobs (' &') only: push exit code + output tail back to you when it finishes."},
                       "heartbeat_timeout": {"type": "integer", "description": "Bg jobs only: alert you once if the job produces no output for N secs. Never kills (silence != dead)."},
                       "max_runtime": {"type": "integer", "description": "Bg jobs only: wall-clock ceiling (secs); notify + auto-kill on trip. The only output-independent kill."},
    ...
    {"name": "bg_status", "tier": "read",
     "description": "Poll the background jobs THIS session started with a trailing ' &' on run_command: returns state (running/exited/killed), runtime, and recent output. No pid = list all; a pid = just that one. A job that's gone usually means it finished, the session ended, or a max_runtime tripped. (Does NOT show dispatched workers - use dispatch_worker action='status' for those.)",
     "parameters": {"type": "object",
